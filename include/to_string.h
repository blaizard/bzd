...
namespace bzd
{
	namespace impl
	{
		namespace to_string
		{
			...
			constexpr void integer(interface::String& str, const T& n, const char* const digits = bzd::impl::to_string::digits)
			{
				auto data = str.data();
				SizeType index = 0;
			...
			{
				constexpr const T resolutionList[15] = {1, 0.1, 0.01, 0.001, 0.0001, 0.00001, 0.000001,
				...
				bzd::impl::to_string::integer(buffer, static_cast<int>(roundedNumber));
				stream.write(buffer);
	...
	constexpr void toStringHex(bzd::OStream& stream, const T& data, const char* const digits = bzd::impl::to_string::digits)
	{
		...
		bzd::impl::to_string::integer<16>(buffer, data, digits);
		stream.write(buffer);
	}
