...
namespace bzd
{
	namespace impl
	{
		...
		public:
			constexpr explicit Queue(const bzd::Span<Element> data) noexcept
					: pool_(data), first_(PoolType::npos), last_(PoolType::npos)
			{
			}
			...
			{
				return (first_ == PoolType::npos);
			}

			constexpr T& front() noexcept { bzd::assert::isTrue(!empty()); return pool_[first_].container_; }
			constexpr const T& front() const noexcept { bzd::assert::isTrue(!empty()); return pool_[first_].container_; }

			constexpr T& back() noexcept { bzd::assert::isTrue(!empty()); return pool_[last_].container_; }
			constexpr const T& back() const noexcept { bzd::assert::isTrue(!empty()); return pool_[last_].container_; }
