...
namespace bzd
{
	namespace impl
	{
		...
		public:
			constexpr explicit SingleLinkedPool(const bzd::Span<Element> data) noexcept
					: Parent(data), free_(0)
			{
			...
			constexpr CapacityType getIndex(const Element& element) const noexcept
			{
				...
				bzd::assert::isTrue(reinterpret_cast<bzd::IntPtrType>(&element) < reinterpret_cast<bzd::IntPtrType>(Parent::data() + Parent::size()));
