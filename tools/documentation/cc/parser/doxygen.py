...
kindToKeep = ["struct", "class", "variable",
        ...
        if len(parameterlistList):
            for parameterlist in parameterlistList:
                ...
                for parameteritem in parameterlist.findall("parameteritem"):
                    ...
                    if parameteritem.find("parameternamelist") is not None:
                        param["name"] = ", ".join(text for text in parameteritem.find(
                            "parameternamelist").itertext() if text.strip())
            ...
            for programlisting in programlistingList:
                ...
                for codeline in programlisting.findall("codeline"):
                    codeBlock += " ".join(text for text in codeline.itertext()
                                          if text.strip()) + "\n"
                codeBlock += "</code>"
                textList.append(codeBlock)
        else:
            textList.append(
                "".join(text for text in para.itertext() if text.strip()))
...
class DoxygenParser:
    ...
    def parseValue(self, rules, value, key=None):
        if value and "values" in rules:
            ...
            if isinstance(rules["values"], list):
                ...
                acceptedValues = [
                    v for k, v in rules["values"].items() if k == value.lower()]
            if len(acceptedValues) == 1:
                value = acceptedValues[0]
            else:
                self.assertTrue(
                    True if "others" in rules and rules["others"] == "ignore" else False, "Value '{}' not accepted for key '{}'", value, key)
                return (key, None)

        if "key" in rules:
            key = rules["key"]
        self.assertTrue(
            key != None, "The 'key' attribute for rule '{}' is missing", rules)
        return (key, value)
        ...
        if value != None:
            ...
            if key in data["__info__"]:
                self.assertTrue(data["__info__"][key] == value,
                                "Overriding key '{}' with a different value '{}' vs '{}': {}", key, data["__info__"][key], value, data)
    ...
    def addMemberList(self, data, key):
        ...
        self.assertTrue(isinstance(
            data["__info__"][key], list), "The key '{}' must be a list", key)
    ...
    def addMemberDict(self, data, key):
        ...
        if key not in data["__info__"]:
            data["__info__"][key] = {}
        self.assertTrue(isinstance(
            data["__info__"][key], dict), "The key '{}' must be a dictionary", key)
        return data["__info__"][key]
    ...
    def parseElement(self, element, definition, data={}):
        if "__attrs__" in definition:
            for key, rules in definition["__attrs__"].items():
                ...
                (key, value) = self.parseValue(
                    rules, element.get(key, None), key)
    ...
    def normalizeName(self, data):
        if self.assertTrueWarning("__info__" in data, "Attribute '__info__' is missing from member: {}", data):
            return
        self.assertTrue(
            "name" in data["__info__"], "Attribute 'name' is missing from member: {}", data)
    ...
    def mergeMember(self, member1, member2):
        self.assertTrue(type(member1) == type(
            member2), "Cannot merge different types {} vs {}", member1, member2)
        ...
        # Merge 2 list
            ...
            self.assertTrue(
                member1 == member2, "Trying to merge different members {} vs {}", member1, member2)
    ...
    def removeNestedKeyword(self, definition, keyword):
        if isinstance(definition, dict):
            if keyword in definition:
                self.assertTrue(isinstance(
                    definition[keyword], dict), "Sub definition under '{}' must be a dict", keyword)
                    ...
                    def getInheritance(inheritanceCurrent, inheritanceList, visibility="public"):
                        if inheritanceCurrent:
                            for child in inheritanceCurrent.get("children", []):
                                ...
                                self.assertTrue(
                                    linkId != None, "Missing link ID: {}", inheritanceCurrent)
                                inheritanceNext = member["inheritance"].get(
                                    linkId, None)
                                if inheritanceNext:
                                    inheritanceList.append({"visibility": visibility, "link": inheritanceNext.get(
                                        "link", None), "id": inheritanceNext.get("name", None)})
                                    getInheritance(
                                        inheritanceNext, inheritanceList, visibility)
                    inheritanceList = []
        ...
        for namespace, members in groups.items():
            for member in members:
                if "description.param" in member:
                    self.mergeMember(
                        member, {"args": member["description.param"]})
                    del member["description.param"]
                if "description.templateparam" in member:
                    self.mergeMember(
                        member, {"template": member["description.templateparam"]})
                    ...
                    for element in root.findall(name):
                        ...
                        if "__type__" in definition:
                            ...
                            if "list" in definition["__type__"]:
                                self.assertTrue(isinstance(
                                    current, list), "Data must be a list '{}'", current)
                                current.append({})
                                current = current[len(current) - 1]
                            if "dict" in definition["__type__"]:
                                self.assertTrue(isinstance(
                                    current, dict), "Data must be a dictionary '{}'", current)
                        ...
                        if "__type__" in definition and "member" in definition["__type__"]:
                            ...
                            self.assertTrue(
                                "__key__" in current["__info__"], "Data must have a key __key__: {}", current)
