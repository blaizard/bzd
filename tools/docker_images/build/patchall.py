...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Update the rpath of all files within a directory."
    )
    parser.add_argument(
        "path", help="The directory in which the rpath should be updated."
    )
    ...
    for path, libs in executables.items():
        for lib in libs:
            if lib in libraries:
                origin = pathlib.Path("$ORIGIN") / os.path.relpath(
                    libraries[lib], path.parent
                )
                subprocess.run(
                    ["patchelf", "--remove-rpath", directory / path], check=True
                )
                subprocess.run(
                    [
                        "patchelf",
    ...
    for lib in sorted(unassigned):
        print(
            f"- {lib} ({len(unassigned[lib])} files): {', '.join([str(p) for p in unassigned[lib]])[:100]}..."
        )
