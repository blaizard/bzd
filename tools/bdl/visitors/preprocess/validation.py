...
class Validation(VisitorBase[None]):
	...
	def visitNestedEntities(self, entity: Nested, result: None) -> None:
		...
		if entity.type == "interface":
			...
			entity.assertTrue(condition=interfaceCategories.issubset({"method", "expression"}), message="Interfaces can only expose methods and expressions as interface.")
			entity.assertTrue(condition=configCategories.issubset({"expression"}), message="Interfaces configuration can only contain expressions.")
			entity.assertTrue(condition=len(compositionCategories) == 0, message="Interfaces cannot have nested composition.")

		elif entity.type == "struct":
			entity.assertTrue(condition=entity.isName, message="Structures must have a valid name.")
			entity.assertTrue(condition=interfaceCategories.issubset({"expression", "enum", "nested", "using"}), message="Structures only accept expressions, enums, nested structs or using statements.")
			entity.assertTrue(condition=len(configCategories) == 0, message="Structures cannot have configuration.")
			entity.assertTrue(condition=len(compositionCategories) == 0, message="Structures cannot have nested composition.")

		elif entity.type == "component":
			entity.assertTrue(condition=entity.isName, message="Components must have a valid name.")
			entity.assertTrue(condition=interfaceCategories.issubset({"method"}), message="Components can only expose methods as interface.")
			entity.assertTrue(condition=configCategories.issubset({"expression"}), message="Components configuration can only contain expressions.")
			entity.assertTrue(condition=compositionCategories.issubset({"expression"}), message="Components composition can only contain expressions.")
			...
			entity.assertTrue(condition=interfaceCategories.issubset({"expression"}), message="Compositions can only contain expressions.")
			entity.assertTrue(condition=len(configCategories) == 0, message="Compositions cannot have configuration.")
			entity.assertTrue(condition=len(compositionCategories) == 0, message="Compositions cannot have nested composition.")
