...
class Manifest():
        ...
        while dependencyGraph:
            ...
            for interface, data in dependencyGraph.items():
                if all([dep not in dependencyGraph for dep in data]):
                    registryList.append(
                        {"interface": interface, "objects": registryEntries[interface]["objects"]})
                else:
                    newDependencyGraph[interface] = data
            if registryListInitialLen == len(registryList):
                raise Exception("Circular depedency detected between interfaces: {}.".format(
                    ", ".join(dependencyGraph.keys())))
    ...
    def merge(self, data, path):
        ...
        try:
            ...
            raise Exception("Error while merging ({}): {}".format("; ".join(
                ["{}: '{}'".format(str(key), str(text)) for key, text in context.items() if text]), e))
    ...
    def process(self):
        ...
        for identifier in self.data.get("objects", {}).keys():
            try:
                ...
                raise Exception(
                    "Error while processing object {}: {}".format(identifier, e))
        ...
        for identifier in self.data.get("interfaces", {}).keys():
            try:
                ...
                raise Exception(
                    "Error while processing interface {}: {}".format(identifier, e))
        ...
        for identifier in self.data.get("artifacts", {}).keys():
            try:
                ...
                raise Exception(
                    "Error while processing artifact {}: {}".format(identifier, e))
    ...
    def _mergeAndValidate(dst, src, validation, context, path):
        ...
        for key, value in src.items():
            ...
            if isinstance(vData, dict):
                assert isinstance(
                    value, dict), "Key '{}' must contain a dictionary type object.".format(key)
                ...
                # Go further
                Manifest._mergeAndValidate(
                    dst[key], value, vData, context, path)
                ...
                if key not in dst:
                    ...
                    assert dst[key] == value, "Conflicting values: '{}' != '{}'".format(
