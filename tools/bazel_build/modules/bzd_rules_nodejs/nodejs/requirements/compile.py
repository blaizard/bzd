...
class Packages:
    ...
    def add(self, package: Package) -> None:
        assert package.package not in self.packages, (
            f"Package '{package.package}' is already added to the dictionary."
        )
    ...
    def getMatching(self, name: str, constraints: str) -> Package:
        ...
        assert maybeVersion is not None, (
            f"Couldn't find a matching version for {name} with {matcher} and {possibleVersions}."
        )
...
class RequirementsFactory:
    ...
    def packagesParserV3(packageLock: Json) -> Packages:
        ...
        packagesByName: typing.Dict[
            str, typing.Dict[Package, typing.List[typing.Dict[str, typing.Any]]]
        ] = {}
        for path, data in packageLock["packages"].items():
            ...
            dependencies = data.get("dependencies", {}) | data.get(
        ...
        for name, packageVersions in packagesByName.items():
            for package, dependencies in packageVersions.items():
                for dependency in dependencies:
                    ...
                    versionsPackages = {
                        p.version: p for p in packagesByName[dependency["name"]].keys()
                    }
                    maybeVersion = dependency["matcher"].matchLatest(
                        versionsPackages.keys()
                    )
                    assert maybeVersion is not None, (
                    ...
                    if dependency["alias"]:
                        package.dependencies.add(
                            versionsPackages[maybeVersion].makeAlias(
        ...
        for name, maybeConstraints in requirements.items():
            ...
            for d in packages:
                dependencies[d.package] = (
                    ...
                    | ({} if d.os is None else {"os": d.os})
                    | ({} if d.cpu is None else {"cpu": d.cpu})
                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Compile requiments into a deps lock file."
    )
    parser.add_argument(
        "--output", type=pathlib.Path, help="The output path for the generated file."
    )
    parser.add_argument(
        "--npm", type=pathlib.Path, help="The path of the pnpm tool.", required=True
    )
    parser.add_argument(
        "srcs", type=pathlib.Path, nargs="+", help="Requirement inputs."
    )
    ...
    requirements = {
        ...
        for k, v in parseRequirements(path.read_text()).items()
    }
    ...
    if args.output:
        output = pathlib.Path(args.output).resolve(strict=True)
