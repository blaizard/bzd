...
class Extensions:
    ...
    def execute(self, m: re.Match[str]) -> str:
        ...
        if len(set(executables)) != 1:
            raise Exception(
                f"All executables must point to the same file: {str(matches)}."
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output", type=pathlib.Path, help="Output path for the preprocessed markdown."
    )
    parser.add_argument(
        "--extension",
        type=pathlib.Path,
