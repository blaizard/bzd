...
def navigationToMkDocsList(
    root: pathlib.Path, navigation: typing.List[typing.Tuple[str, typing.Any]]
) -> typing.List[str]:
    content = []
...
def navigationToPathList(
    navigation: typing.List[typing.Tuple[str, typing.Any]],
...
def searchForEntryPoint(
    output: pathlib.Path, navigation: typing.List[typing.Tuple[str, typing.Any]]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--navigation", type=str, default="[]", help="Navigation as a json string."
    )
    parser.add_argument(
        "--mkdocs", type=pathlib.Path, default="mkdocs", help="Path of the mkdocs tool."
    )
    parser.add_argument(
        "--json",
        type=pathlib.Path,
    ...
    output = template.format(
        navigation="\n".join(
            navigationToMkDocsList(root=args.root, navigation=navigation)
        ),
    ...
    with tempfile.TemporaryDirectory() as pathStr:
        ...
        if not (path / "index.html").is_file():
            ...
            assert maybeEntryPoint is not None, (
                "No entry point found for this documentation."
            )
