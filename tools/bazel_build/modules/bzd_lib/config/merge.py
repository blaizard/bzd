...
class Config:
    ...
    def addDict(
        self, data: typing.Dict[str, typing.Any], source: str, failOnConflict: bool
    ) -> None:
        self._addData(data, source, failOnConflict)
    ...
    def _addData(
        ...
        def policy(
            originalValue: typing.Any, newValue: typing.Any, keys: typing.List[str]
        ) -> None:
            if originalValue is None or newValue is None:
                return
            if isinstance(originalValue, list) and not isinstance(newValue, list):
                fatal(
                    f"The override of a list should be a list, see key '{'.'.join(keys)}' from '{source}'."
                )
            return
        ...
        try:
            ...
            fatal(
                f"The key {e} from '{source}' was already defined by another base configuration."
            )
...
if __name__ == "__main__":
    ...
    assert len(workspaceStatusKeysUsed) == len(args.workspaceStatusKeys), (
        "Some workspace status keys were not found."
    )
