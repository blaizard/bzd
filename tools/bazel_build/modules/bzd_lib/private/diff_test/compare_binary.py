...
class VisitorPrint:
    ...
    def toHexAsString(
        self, diff: DiffContent, padding: typing.Optional[int] = None
    ) -> str:
        ...
        if self.color:
            content = [
                "{}{:02x}\033[0m".format(VisitorPrint.getColorAsString(isDiff), byte)
    ...
    def toTextAsString(
        self, diffContent: DiffContent, padding: typing.Optional[int] = None
    ) -> str:
        ...
        if self.color:
            ...
            content = [
                "{}".format(chr(byte) if byte in range(32, 127) else ".")
                for byte, _ in diffContent
            ]
        if padding is not None:
    ...
    @staticmethod
    def chunkedIterator(
        diffContent1: DiffContent, diffContent2: DiffContent, chunkSize: int
    ) -> typing.Generator[typing.Tuple[DiffContent, DiffContent], None, None]:
    ...
    def visitBinary(self, delta: Delta) -> None:
        ...
        for chunk1, chunk2 in VisitorPrint.chunkedIterator(
            delta.diff1, delta.diff2, chunkSize
        ):
            left.append(
                formatLine.format(
    ...
    def visitText(self, delta: Delta) -> None:
        ...
        for chunk1, chunk2 in VisitorPrint.chunkedIterator(
            delta.diff1, delta.diff2, self.width
        ):
...
class Diff:
    ...
    def _preprocess(self) -> None:
        ...
        for b in lcs:
            nextIndex1 = self.content1.find(b, index1)
            nextIndex2 = self.content2.find(b, index2)
    ...
    def visit(
        self,
        visitor: typing.Callable[[Delta], None],
        context: int = 4,
        distance: int = 8,
    ) -> None:
        """Visit the diff.
        ...
        for index1, nextIndex1, index2, nextIndex2 in self.diffs:
            ...
            if delta.isEmpty:
                ...
                delta.addDiff1(
                    self.content1[
                        index1 - min(index1 - previousIndex1, context) : index1
                    ],
                    0,
                )
                delta.addDiff2(
                    self.content2[
                        index2 - min(index2 - previousIndex2, context) : index2
                    ],
                    0,
                )
            ...
            # Diff exists but close, can concatenate.
                ...
                delta.addDiff1(
                    self.content1[
                        index1 - min(index1 - previousIndex1, distance) : index1
                    ],
                    0,
                )
                delta.addDiff2(
                    self.content2[
                        index2 - min(index2 - previousIndex2, distance) : index2
                    ],
                    0,
                )
...
def compare(file1: pathlib.Path, file2: pathlib.Path) -> bool:
    ...
    visitor = VisitorPrint(
        color=True, width=80, file1=file1.as_posix(), file2=file2.as_posix()
    )
