...
if __name__ == "__main__":
    ...
    startTime = time.time()
    result = localCommand(
        cmds=[str(args.executable)] + args.rest,
        ignoreFailure=True,
        stdout=True,
        stderr=True,
    )
    ...
    if args.expected_returncode is not None:
        nbChecks += 1
        assert result.getReturnCode() == args.expected_returncode, (
            f"The return code {result.getReturnCode()} is different from the expected return code {args.expected_returncode}."
        )

    if args.unexpected_returncode is not None:
        nbChecks += 1
        assert result.getReturnCode() != args.unexpected_returncode, (
            f"The return code {result.getReturnCode()} was not expected."
        )

    if args.expected_output is not None:
        nbChecks += 1
        assert (
            args.expected_output in result.getStdout()
            or args.expected_output in result.getStderr()
        ), (
    ...
    if args.min_duration is not None:
        ...
        assert duration >= args.min_duration, (
            f"The execution time was less than the expected minimal duration {args.min_duration}s vs {duration}s."
        )

    if args.max_duration is not None:
        nbChecks += 1
        assert duration <= args.max_duration, (
            f"The execution time was more than the expected maximal duration {args.max_duration}s vs {duration}s."
        )
    ...
    assert nbChecks > 0, (
        f"Please set at least one of the attributes: {', '.join(attributes)}."
    )
