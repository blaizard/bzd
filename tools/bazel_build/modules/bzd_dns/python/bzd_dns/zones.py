...
class Zones:
    ...
    def add(self, data: ZonesJson) -> None:
        ...
        assert isinstance(data, dict), (
            f"The zone json zones representation must contain a valid dictionary: {data}"
        )
        for subdomain, records in data.items():
            assert isinstance(records, list), (
                f"Records for subdomain '{subdomain}' must be a list: {records}"
            )
    ...
    def toJson(self) -> ZonesJson:
        ...
        return {
            subdomain: [record.data for record in records]
