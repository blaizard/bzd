...
class Builder:
    ...
    def add(
        self, attribute: str, yaml: typing.Optional[str] = None, formatStr: str = "{}"
    ) -> "Builder":
        if attribute in self.record.data:
            self.data[yaml if yaml else attribute] = formatStr.format(
                self.record.data[attribute]
            )
        return self
        ...
        if value is not None:
            if data.get(key) is not None:
                assert value == data.get(key), (
                    f"Value for '{key}' conflicts between '{value}' and '{data.get(key)}'."
                )
            data[key] = value
