...
class Record:
    ...
    def __init__(self, data: RecordsJson) -> None:
        ...
        for attribute in self.mandatory + ["type"]:
            assert attribute in self.data, (
                f"Missing mandatory attribute '{attribute}' in {self}."
            )
        ...
        for attribute in self.data.keys():
            assert attribute in checked, (
                f"The attribute '{attribute}' is not valid for {self}."
            )
    ...
    def make(data: RecordsJson) -> "Record":
        ...
        assert data["type"] in typesToClass, (
            f"Unsupported record type '{data['type']}'."
        )
    ...
    def visit(self, visitor: typing.Any) -> None:
        ...
        assert hasattr(visitor, methodName), (
            f"The visitor is missing a method named '{methodName}'."
        )
