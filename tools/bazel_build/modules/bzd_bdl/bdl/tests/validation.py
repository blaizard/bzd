...
class TestRun(unittest.TestCase):
    ...
    def testContracts(self) -> None:
        ...
        with self.assertRaisesRegex(Exception, r"missing mandatory"):
            Object.fromContent(
                content="using MyType = Integer [min]; struct A { temp = MyType; }",
                objectContext=ObjectContext(resolve=True),
            )
        Object.fromContent(
            content="using MyType = Integer [min(2)]; struct A { temp = MyType; }"
        )
        with self.assertRaisesRegex(Exception, r"not expected"):
            Object.fromContent(
                content="using MyType = Integer [min(2,3)]; struct A { temp = MyType; }",
                objectContext=ObjectContext(resolve=True),
            )
        ...
        with self.assertRaisesRegex(Exception, r"missing mandatory"):
            Object.fromContent(
                content="using MyType = Integer [capacity];",
                objectContext=ObjectContext(resolve=True),
            )
        with self.assertRaisesRegex(Exception, r"not expected"):
            Object.fromContent(
                content="using MyType = Integer [capacity(1,2)];",
                objectContext=ObjectContext(resolve=True),
            )
        with self.assertRaisesRegex(Exception, r".*lower.*"):
            Object.fromContent(
                content="using MyType = Integer [capacity(0)];",
                objectContext=ObjectContext(resolve=True),
            )
        Object.fromContent(
            content="using MyType = Integer [capacity(10)];",
            objectContext=ObjectContext(resolve=True),
        )

        # Init
        Object.fromContent(
            content="interface temp { method hello() [init]; }",
            objectContext=ObjectContext(resolve=True),
        )
