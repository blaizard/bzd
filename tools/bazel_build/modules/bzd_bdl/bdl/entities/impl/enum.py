...
class Enum(Entity):
    ...
    def toLiteral(
        ...
        value = args["value"]
        assert isinstance(value, EntityExpression), (
            f"The value must be of type 'EntityExpression', not {value}."
        )
        assert value.isLiteral, f"It must have a literal type: {value}."
        literal = value.literalNative
        assert isinstance(literal, dict) and literal.get("type") == "enum", (
            f"The value must be of type 'enum', not '{literal}'."
        )
    ...
    def getConfigValues(self, resolver: typing.Any) -> Parameters:
        ...
        assert maybeFirst, (
            f"This enum '{self.fqn}' does not have any values: '{self.values}'"
        )

        # Create the parameter.
        params = Parameters(element=self.element, NestedElementType=Expression)
        element = (
            ElementBuilder().setAttr("category", "expression").setAttr("name", "value")
        )
    ...
    def __repr__(self) -> str:
        return self.toString(
            {"name": self.name, "values": ", ".join([e.name for e in self.values])}
        )
