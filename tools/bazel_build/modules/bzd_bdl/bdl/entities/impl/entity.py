...
class Entity:
    ...
    def toLiteral(
        self, args: typing.Dict[str, "EntityExpression"]
    ) -> typing.Optional[str]:
    ...
    def addParents(self, fqn: typing.Optional[str], parents: typing.List[str]) -> None:
        ...
        ElementBuilder.cast(self.element, ElementBuilder).setAttr(
            "parents", ";".join(updatedParents)
        )
    ...
    def _setLiteral(
        self, value: typing.Optional[typing.Union[str, LiteralExtended]]
    ) -> None:
    ...
    def literalNative(
        self,
    ) -> typing.Optional[typing.Union[int, float, str, bool, LiteralExtended]]:
    ...
    def getEntityUnderlyingTypeResolved(self, resolver: "Resolver") -> "EntityType":
        ...
        entity = resolver.getEntityResolved(fqn=self.underlyingTypeFQN).assertValue(
            element=self.element
        )
        assert entity.isRoleType, (
            f"The role of the underlying entity '{entity}' with FQN {self.underlyingTypeFQN} of the entity {self} must be a type."
        )
        return entity
    ...
    def isExecutor(self) -> bool:
        ...
        return maybeExecutor is not None and not maybeExecutor.isValue
    ...
    def getConfigTemplateTypes(self, resolver: typing.Any) -> Parameters:
        ...
        if self.underlyingTypeFQN:
            underlyingType = resolver.getEntityResolved(
                fqn=self.underlyingTypeFQN
            ).assertValue(element=self.element)
            params = Parameters(
    ...
    def getConfigValues(self, resolver: "Resolver") -> Parameters:
        ...
        if self.underlyingTypeFQN:
            ...
            underlyingTypeEntity = resolver.getEntityResolved(
                fqn=self.underlyingTypeFQN
            ).assertValue(element=self.element)
        ...
        for fqn in reversed(typeFqns + self.getParents()):
            parentEntity = resolver.getEntityResolved(fqn=fqn).assertValue(
                element=self.element
            )
            ...
            for entity in parentParams:
                typing.cast("Expression", entity).resolve(
                    resolver=resolver.make(namespace=parentEntity.namespace)
                )
    ...
    def resolve(self, resolver: "Resolver") -> None:
        ...
        if maybeSchema:
            resultValidate = Validation(
                schema=[maybeSchema], args={"resolver": resolver}
    ...
    def error(
        self, message: str, element: typing.Optional[Element] = None, throw: bool = True
    ) -> AssertionResult:
    ...
    def toString(
        self, attrs: typing.MutableMapping[str, typing.Optional[str]] = {}
    ) -> str:
        entities = [
...
class EntityExpression(Entity):
    ...
    def isVarArgs(self) -> bool:
        return self.element.isAttr("name") and self.element.getAttr(
            "name"
        ).value.endswith("...")
    ...
    def isLValue(self) -> bool:
        ...
        return (
            self.isRoleValue and self.isSymbol and self.underlyingValueFQN is not None
        )

    def toParametersResolvedItem(
        ...
        return self.toString(
            {
                ...
                "literal": str(self.literal) if self.isLiteral else None,
