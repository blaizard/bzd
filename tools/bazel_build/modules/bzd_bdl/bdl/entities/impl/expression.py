...
class Expression(EntityExpression):
    ...
    def isInterfaceType(self) -> bool:
        return self.element.isAttr("interface") and bool(
            self.element.getAttr("interface").value
        )
    ...
    def fragments(self) -> typing.List[ExpressionFragment]:
        ...
        return [
            ExpressionFragment.make(element)
            for element in self.element.getNestedSequenceOrEmpty("fragments")
        ]
    ...
    def makeResolver(self, symbols: "SymbolMap", **kwargs: typing.Any) -> "Resolver":
        ...
        if self.isFQN:
            return symbols.makeResolver(
                namespace=self.namespace, this=self.this, **kwargs
            )
    ...
    def resolveFragments(self, resolver: "Resolver") -> None:
        ...
        for kind, operators in operatorsPrecedence:
            ...
            while index < len(fragments):
                ...
                fragmentNext: typing.Optional[ExpressionFragment] = (
                    fragments[index + 1] if index + 1 < len(fragments) else None
                )
                if (
                    isinstance(fragment, OperatorFragment)
                    and fragment.operator in operators
                ):
                    # Look for operators that are preceded with another operator or at the beginning.
                    isUnary = kind == "unary"
                    isUnary &= fragmentPrevious is None or isinstance(
                        fragmentPrevious, OperatorFragment
                    )
                    isUnary &= fragmentNext is not None and not isinstance(
                        fragmentNext, OperatorFragment
                    )
                    if isUnary:
                        assert fragmentNext
                    ...
                    isBinary &= fragmentPrevious is not None and not isinstance(
                        fragmentPrevious, OperatorFragment
                    )
                    isBinary &= fragmentNext is not None and not isinstance(
                        fragmentNext, OperatorFragment
                    )
                    if isBinary:
    ...
    def resolve(self, resolver: "Resolver") -> None:
        ...
        if self.contracts.validationForValue:
            validation = Validation(
                schema=[self.contracts.validationForValue], args={"resolver": resolver}
            )
