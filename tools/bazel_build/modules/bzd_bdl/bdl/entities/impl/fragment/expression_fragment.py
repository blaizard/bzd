...
class ExpressionFragment(EntityExpression):
    ...
    def make(element: Element) -> "ExpressionFragment":  # type: ignore
        ...
        ExpressionFragment(element).error(
            message=f"Unsupported expression fragment for element: '{element}'."
        )
    ...
    def unary(self, operator: "OperatorFragment") -> None:
        ...
        if isinstance(literal, (int, float)):
            if operator.operator == "+":
                ...
                self.error(
                    f"Unsupported unary operator with number '{operator.operator}'."
                )
            return
        ...
        if regexpr.isValid:
            if operator.operator == "+":
                ...
                self.error(
                    f"Unsupported unary operator with regexpr '{operator.operator}'."
                )

        self.error(f"Unsupported unary operator '{operator.operator}'.")

    def binary(
        self, operator: "OperatorFragment", fragment: "ExpressionFragment"
    ) -> None:
        ...
        if isinstance(literal1, (int, float)) and isinstance(literal2, (int, float)):
            if operator.operator == "+":
                ...
                self.error(
                    f"Unsupported binary operator with numbers '{operator.operator}'."
                )
            return
...
class SymbolFragment(ExpressionFragment):
    ...
    def resolve(self, resolver: "Resolver") -> None:
        ...
        if entity.isRoleValue:
            self.assertTrue(
                condition=not self.isParameters,
    ...
    def _resolveAndValidateParameters(
        ...
        parameters.makeParametersResolved(
            name="argument", resolver=resolver, expected=expectedParameters
        )
        ...
        if Category.component in parameterTypeCategories:
            ...
            self.assertTrue(
                condition=category
                in [Category.component, Category.method, Category.builtin],
                message=f"{resolvedTypeEntity}: Components as parameters are not allowed for entities of category '{category}'.",
            )
...
class PresetFragment(SymbolFragment):
    def resolve(self, resolver: "Resolver") -> None:
        presets = {
            "out": {"interface": "bzd.OStream", "symbol": "target.out"},
            "in": {"interface": "bzd.IStream", "symbol": "target.in"},
            "timer": {"interface": "bzd.Timer", "symbol": "target.timer"},
            "clock": {"interface": "bzd.Clock", "symbol": "target.clock"},
            "network.tcp.client": {
