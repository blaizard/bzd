...
class Contracts:
    ...
    def __init__(
        self, element: Element, sequenceKind: typing.Optional[str] = None
    ) -> None:
    ...
    def assertOnly(self, kinds: typing.Set[str]) -> None:
        ...
        for contract in self:
            Error.assertTrue(
                ...
                message=f"The contract type '{contract.type}' is not allowed for this entity, only the following(s) are supported: {', '.join(kinds)}.",
            )
    ...
    def validationForValue(self) -> typing.Optional[str]:
        ...
        """
        content = [
            str(contract) for contract in self if contract.type in AllContracts.forValue
        ]
        return " ".join(content) if content else None
    ...
    def validationForTemplate(self) -> typing.Optional[str]:
        ...
        content = [
            str(contract)
            for contract in self
            if contract.type in AllContracts.forTemplate
        ]
        return " ".join(content) if content else None
    ...
    def validationForEntity(self) -> typing.Optional[str]:
        ...
        content = [
            str(contract)
            for contract in self
            if contract.type in AllContracts.forEntity
        ]
        return " ".join(content) if content else None
    ...
    def merge(self, contracts: "Contracts") -> None:
        ...
        for contract in reversed([*contracts]):
            ...
            if existing is not None:
                ...
                assert contractTraits, (
                    "All contracts should have been validated already."
                )
                try:
                    contract = contractTraits.resolveConflict(
                        base=contract, derived=existing
                    )
                except Exception as e:
                    Error.handleFromElement(
                        element=self.element, message=str(e), throw=False
                    ).extend(
                        element=contracts.element, message="Base contract is here."
                    )
        ...
        if sequence:
            for element in sequence:
                if Contract(element).type == kind:
                    SequenceBuilder.cast(sequence, SequenceBuilder).removeElement(
                        element
                    )
