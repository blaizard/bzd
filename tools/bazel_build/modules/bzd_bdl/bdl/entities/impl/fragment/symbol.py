...
class Symbol:
    ...
    def resolve(self, resolver: "Resolver", maybeValue: bool = False) -> "EntityType":
        ...
        fqns = resolver.resolveFQN(name=self.fqn).assertValue(
            element=self.element, attr=self.kindAttr
        )
        ElementBuilder.cast(self.element, ElementBuilder).setAttr(
            f"{self.kindAttr}_resolved", ";".join(fqns)
        )
        ...
        ElementBuilder.cast(self.element, ElementBuilder).setAttr(
            f"{self.kindAttr}_category", entity.category.value
        )

        if self.underlyingTypeAttr is not None and entity.underlyingTypeFQN is not None:
            ElementBuilder.cast(self.element, ElementBuilder).setAttr(
                self.underlyingTypeAttr, entity.underlyingTypeFQN
            )
        ...
        if not configTypes:
            ...
            self.templates.makeParametersResolved(
                name=self.templateAttr, resolver=resolver, expected=configTypes
            )
            ...
            arguments = self.templates.getValuesOrTypesAsDict(
                resolver=resolver, varArgs=False
            )
    ...
    def getEntityResolved(self, resolver: "Resolver") -> "EntityType":
        ...
        """
        return resolver.getEntityResolved(fqn=self.kind).assertValue(
            element=self.element, attr=self.kindAttr
        )
    ...
    def getEntityUnderlyingTypeResolved(self, resolver: "Resolver") -> "EntityType":
        ...
        return resolver.getEntity(fqn=self.underlyingTypeFQN).assertValue(
            element=self.element, attr=self.kindAttr
        )

    def getThisResolved(self, resolver: "Resolver") -> "EntityType":
        return resolver.getEntity(fqn=self.this).assertValue(
            element=self.element, attr=self.kindAttr
        )
    ...
    def templates(self) -> "Parameters":
        ...
        return Parameters(
            element=self.element, NestedElementType=Using, nestedKind=self.templateAttr
        )
    ...
    def kinds(self) -> typing.List[str]:
        return (
            self.element.getAttr(f"{self.kindAttr}_resolved").value.split(";")
            if self.element.isAttr(f"{self.kindAttr}_resolved")
            else [self.fqn]
        )
    ...
    def isConvertible(self, resolver: "Resolver", to: "Symbol") -> bool:
        ...
        for parentFQN in typeFrom.getUnderlyingTypeParents(resolver=resolver):
            entity = resolver.getEntityResolved(fqn=parentFQN).assertValue(
                element=typeFrom.element
            )
            if entity == typeTo:
                return True
    ...
    def error(self, message: str, throw: bool = True) -> AssertionResult:
        return Error.handleFromElement(
            element=self.element, attr=self.kindAttr, message=message, throw=throw
        )

    def assertTrue(
        self, condition: bool, message: str, throw: bool = True
    ) -> AssertionResult:
...
class Visitor(VisitorDepthFirstBase[typing.List[str], str]):
    ...
    def __init__(self, symbol: Symbol) -> None:
        ...
        self.result = self.visitSymbol(
            symbol=symbol, nested=template, parameters=symbol.templateResolved
        )
