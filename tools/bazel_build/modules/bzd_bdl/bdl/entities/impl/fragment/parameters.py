...
class ParametersCommon:
    ...
    def getUnderlyingTypeCategories(
    ...
    def items(
        self, includeVarArgs: bool
    ) -> typing.Iterator[typing.Tuple[Key, "EntityExpression"]]:
        for key, entity, _ in self.itemsMetadata(includeVarArgs=includeVarArgs):
            yield key, entity

    def itemsMetadata(
        self, includeVarArgs: bool
    ) -> typing.Iterator[typing.Tuple[Key, "EntityExpression", Data]]:
    ...
    def makeValidation(
        self, resolver: "Resolver", forTemplate: bool
        ...
        for key, expression in self.items(includeVarArgs=True):
            keyStr = (
                "*" if expression.isVarArgs else str(key.index if forTemplate else key)
            )
            maybeContracts = (
                expression.contracts.validationForTemplate
                if forTemplate
                else expression.contracts.validationForValue
            )
...
class Parameters(ParametersCommon):
    ...
    def copy(self) -> "Parameters":
        ...
        parameters = Parameters(
            element=self.element, NestedElementType=self.NestedElementType
        )
    ...
    def getValuesOrTypesAsDict(
        self, resolver: "Resolver", varArgs: bool
    ) -> typing.Dict[str, ResolvedType]:
        """
        Get the values as a dictionary.
        """
        return {
            str(key): expression
            for key, expression, _ in self.itemsMetadata(includeVarArgs=varArgs)
        }
    ...
    def toResolvedSequence(self, resolver: "Resolver", varArgs: bool) -> Sequence:
        ...
        """
        items = [
            metadata for _, _, metadata in self.itemsMetadata(includeVarArgs=varArgs)
        ]
    ...
    def makeParametersResolved(
        self, name: str, resolver: "Resolver", expected: "Parameters"
    ) -> None:
        ...
        ElementBuilder.cast(self.element, ElementBuilder).setNestedSequence(
            f"{name}_resolved", sequenceValues
        )

        sequence = expected.toResolvedSequence(resolver=resolver, varArgs=True)
        sequence += (
            [sequence[-1]] * (len(sequenceValues) - len(sequence))
            if expected.isVarArgs
            else []
        )
        ElementBuilder.cast(self.element, ElementBuilder).setNestedSequence(
            f"{name}_expected", sequence
        )
