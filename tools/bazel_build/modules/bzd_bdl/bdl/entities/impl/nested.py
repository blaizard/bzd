...
from bdl.entities.impl.entity import Entity, Role, EntityExpression
...
class Nested(Entity):
    ...
    def __init__(self, element: Element) -> None:
        ...
        self.assertTrue(
            ...
            message=f"Unsupported nested type: '{self.category}', only the following are supported: {Nested.categoriesAllowed_}",
        )
    ...
    def resolve(self, resolver: typing.Any) -> None:
        ...
        for inheritance in self.inheritanceList:
            ...
            if underlyingType.category in Nested.categoriesAllowed_:
                ...
                self.error(
                    message=f"Inheritance can only be done from a nested class, not '{entity.underlyingTypeFQN}', category '{underlyingType.category}'."
                )
            ...
            if self.category == Category.struct:
                ...
                self.assertTrue(
                    condition=nestedCategory
                    in {Category.interface, Category.component},
                    message=f"A component can only inherits from interface(s), not '{nestedCategory}'.",
                )
            else:
                self.error(
                    message=f"Unsupported inheritance for type: '{self.category}'."
                )
                ...
                if entity.category == Category.expression:
                    ...
                    if entity.isSymbol:
                        ...
                        entity.assertTrue(
                            ...
                            message=f"Only {', '.join(str(c.value) for c in allowed)} can be used as expression in interfaces, not '{entity.symbol.category.value}'",
                        )
