...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-o", "--output", default=None, type=Path, help="Output path of generated file."
    )
    parser.add_argument(
        "--format",
        default="bdl",
    ...
    parser.add_argument(
        "inputs", type=str, nargs="+", help="Input file to be passed to the parser."
    )
        ...
        for source in config.inputs:
            if config.stage == "preprocess":
                ...
                if config.stage == "generate":
                    maybePreprocess = objectContext.findPreprocess(source=source)
                    assert maybePreprocess is not None, (
                        f"Source file '{source}' is not preprocessed."
                    )
                    bdl = objectContext.loadPreprocess(preprocess=maybePreprocess)
                    output = generate(
                        formatType=config.format, bdl=bdl, data=config.data
                    )
