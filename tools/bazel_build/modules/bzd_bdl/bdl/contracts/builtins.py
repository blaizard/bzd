...
def getValueFromSingleParameter_(
    resolver: typing.Any, typeFQN: str, value: typing.Any, defaultValue: str
) -> typing.Any:
    ...
    assert isinstance(resolver, Resolver), (
        f"The resolver is not valid: {str(resolver)}."
    )
        ...
        if value.isLiteral:
            ...
            if value.underlyingValueFQN is not None:
                ...
                assert isinstance(value, Expression), (
                    "An underlying value FQN must point to an expression."
                )
...
class Integer_(Integer):
    def check(self, context: TypeContext) -> None:
        ...
        context.value = getValueFromSingleParameter_(
...
class ContractInteger(ContractTraits):
    def __init__(self) -> None:
        super().__init__(
            name="integer", role=Role.Value | Role.Public, constraint=Integer_
        )
...
class ContractBoolean(ContractTraits):
    def __init__(self) -> None:
        super().__init__(
            name="boolean", role=Role.Value | Role.Public, constraint=Boolean_
        )
