...
class InitConstraint_(Constraint):
    ...
    def check(self, context: TypeContext) -> None:
        ...
        if isinstance(context.value, Entity):
            underlyingTypeFQN = (
                context.args["resolver"]
                .getEntityResolved(context.value.underlyingTypeFQN)
                .value
            )
            assert underlyingTypeFQN.category == Category.method, (
                "'init' constraint applies only to methods."
            )
            assert underlyingTypeFQN.parameters.empty(), (
                "'init' methods cannot take arguments."
            )
...
class ContractInit(ContractTraits):
    ...
    def __init__(self) -> None:
        super().__init__(
            name="init", role=Role.Entity | Role.Public, constraint=InitConstraint_
        )
