...
class Contract:
    ...
    @staticmethod
    def add(
        element: Element, kind: str, values: typing.Optional[typing.List[str]] = None
    ) -> None:
        """
        ...
        ElementBuilder.cast(element, ElementBuilder).pushBackElementToNestedSequence(
    ...
    def valueNumber(self) -> float:
        try:
            ...
            Error.handleFromElement(
                element=self.element, message="Expected a valid number."
            )
    ...
    def valueString(self) -> str:
        ...
        if value is None:
            Error.handleFromElement(
                element=self.element, message="A value must be present."
            )
