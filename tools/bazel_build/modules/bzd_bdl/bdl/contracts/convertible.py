...
class Convertible_(Constraint):
    def install(self, processedSchema: ProcessedSchema, args: typing.List[str]) -> None:
        ...
        def _process(base: str, context: Context) -> None:
            ...
            assert isinstance(context.value, EntityExpression), (
            ...
            assert typeFrom.isConvertible(
                resolver=context.args["resolver"], to=entityTo.symbol
            ), f"Type '{typeFrom}' is not convertible to type '{entityTo.symbol}'."
...
class ContractConvertible(ContractTraits):
    def __init__(self) -> None:
        super().__init__(
            name="convertible", role=Role.Template, constraint=Convertible_
        )
