...
class SymbolTree(EntitySequence):
    ...
    def addEntity(self, entity: EntityType) -> None:
        entity.assertTrue(
            condition=entity.isFQN, message="Entity is missing FQN attribute."
        )
        assert self.symbols.contains(entity.fqn), (
            "This FQN '{}' is not registered in the symbol map.".format(entity.fqn)
        )
    ...
    def fromSerialize(data: typing.List[str], symbols: SymbolMap) -> "SymbolTree":
        ...
        # Checks
        for fqn in data:
            assert symbols.contains(fqn=fqn), (
                "FQN '{}' not found in the symbol map.".format(fqn)
            )
