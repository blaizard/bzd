	...
	def resolveFQN(self, name: str) -> ResolveFQNResult:
		...
		for nextName in unresolvedFQN:
			...
			if entity.underlyingTypeFQN is not None:
				potentialNamespaceFQNs += [entity.underlyingTypeFQN] + entity.getEntityUnderlyingTypeResolved(
					resolver=self
				).getParents()
				...
				if element.isNestedSequence(group.value):
					...
					for nested in element.getNestedSequenceAssert(group.value):
						...
						if isinstance(nestedEntity, Reference):
							nestedEntity = self.getEntityResolved(fqn=nestedEntity.name, exclude=exclude).assertValue(element=element)
						updatedSequence.pushBackElement(nestedEntity.element)
					ElementBuilder.cast(element, ElementBuilder).setNestedSequence(kind=group.value, sequence=updatedSequence)
