...
from bdl.visitor import Group, NestedSequence
...
class Resolver:
    ...
    def make(
        ...
        return expression.makeResolver(
            symbols=self.symbols, target=self.target, memoize=self.memoize
        )
        ...
        if nameFirst == "this":
            ...
            while True:
                ...
                if not potentialNamespace:
                    ending = self.symbols._terminateErrorMessageSimilarFQN(
                        fqn=nameFirst
                    )
                    return ResolveShallowFQNResult.makeError(
                        f"Symbol '{nameFirst}' in namespace '{'.'.join(self.namespace)}' could not be resolved{ending}"
                        if self.namespace
                        else f"Symbol '{nameFirst}' could not be resolved{ending}"
                    )
        ...
        for nextName in unresolvedFQN:
            ...
            for potentialNamespaceFQN in potentialNamespaceFQNs:
                potentialFQN = FQN.fromNamespace(
                    name=nextName, namespace=FQN.toNamespace(potentialNamespaceFQN)
                )
            ...
            if fqn is None:
                ...
                return ResolveFQNResult.makeError(
                    ...
                    if self.namespace
                    else f"Symbol '{nextName}' from '{name}' could not be resolved{ending}"
                )
            fqns.append(fqn)
    ...
    def __repr__(self) -> str:
        ...
        entities = [
            '{}="{}"'.format(key, value) for key, value in attrs.items() if value
        ]
...
class SymbolMap:
    ...
    def contains(
        self, fqn: str, exclude: typing.Optional[typing.List[Group]] = None
    ) -> bool:
        """
    ...
    def items(
        self, groups: typing.Set[Group], startsWith: str = "", depth: int = 0
    ) -> typing.Iterator[typing.Tuple[str, EntityType]]:
        """
    ...
    def insert(
        ...
        if self.contains(fqn=fqn):
            originalElement = (
                self.getEntityResolved(fqn=fqn).assertValue(element=element).element
            )
    ...
    def makeReference(self, fqn: str, group: typing.Optional[Group] = None) -> Element:
        ...
        """
        assert self.contains(fqn=fqn), (
            "The FQN '{}' is not part of the symbol map.".format(fqn)
        )
    ...
    def close(self) -> None:
        ...
        for fqn, entity in self.entities.items():
            ...
            entity.assertTrue(
                ...
                message="Entry '{}' was not properly created before being added to the entities map.".format(
                    fqn
                ),
            )
            ...
            if any([element.isNestedSequence(group.value) for group in NestedSequence]):
                preparedElement = element.copy(
                    ignoreNested=[group.value for group in NestedSequence]
                )
    ...
    def metaToElement(meta: typing.Any) -> Element:
        return Element.fromSerialize(
            element=meta["e"], context=Context(path=Path(meta["p"]))
        )
    ...
    def similarFQN(self, fqn: str) -> typing.List[str]:
        ...
        all_fqns = [
            k for k in [*self.map.keys(), *self.builtins.keys()] if not k.endswith("~")
        ]
                ...
                if element.isNestedSequence(group.value):
                    ...
                    for nested in element.getNestedSequenceAssert(group.value):
                        ...
                        if isinstance(nestedEntity, Reference):
                            nestedEntity = self.getEntityResolved(
                                fqn=nestedEntity.name, exclude=exclude
                            ).assertValue(element=element)
                        updatedSequence.pushBackElement(nestedEntity.element)
                    ElementBuilder.cast(element, ElementBuilder).setNestedSequence(
                        kind=group.value, sequence=updatedSequence
                    )
        ...
        for name in FQN.toNamespace(fqn):
            for namespace in namespaceChoices:
                ...
                if result.value.underlyingTypeFQN is not None:
                    ...
                    for fqn in self.getEntityResolved(
                        result.value.underlyingTypeFQN
                    ).value.getParents():
    ...
    def getChildren(
        self, fqn: str, groups: typing.Set[Group]
    ) -> Result[typing.Dict[str, EntityType]]:
        ...
        for prefix in [result.value.underlyingTypeFQN] + self.getEntityResolved(
            ...
            for fqn, item in self.items(
                groups=groups, startsWith=f"{prefix}.", depth=1
            ):
