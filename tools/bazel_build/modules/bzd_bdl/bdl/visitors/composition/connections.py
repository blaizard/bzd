...
class EndpointId:
    ...
    def fromSymbol(symbol: Symbol) -> "EndpointId":
        symbol.assertTrue(
            ...
            message="The symbol must point to an instance member.",
        )
...
class Connections:
    ...
    def addEndpoint(self, this: EntityExpression, endpoint: EntityExpression) -> None:
        ...
        endpoint.assertTrue(
            condition=endpoint.isName, message="Endpoint must have a name."
        )
        endpoint.assertTrue(
            condition=endpoint.isSymbol, message="Endpoint must have a symbol."
        )
    ...
    def addConnection(self, source: EndpointId, sink: EndpointId) -> None:
        ...
        assert self.endpoints[source].isSource, (
            f"The source '{source}' must not be marked as const."
        )
        assert not self.endpoints[sink].isSource, (
            f"The sink '{sink}' must be marked as const."
        )

        # Sanity checks.
        assert source != sink, (
            f"A connection cannot connect to itself: '{source}' -> '{sink}'."
        )
        assert (
        ...
        assert (
            sourceTypeFQN == sinkTypeFQN
            or sourceTypeFQN == "Any"
            or sinkTypeFQN == "Any"
        ), (
    ...
    def getIdentifiers(
        ...
        entity.assertTrue(
            condition=entity.isSymbol, message="Must be a regexpr of a symbol."
        )
        entity.assertTrue(
            condition=entity.isLValue, message="Must be a reference to another object."
        )

        # If the entity represents a special type.
        entityType = entity.symbol.getEntityUnderlyingTypeResolved(
    ...
    def close(self) -> None:
        ...
        allSources = {
            identifier
            for identifier, metadata in self.endpoints.items()
            if metadata.isSource
        }
        allSinks = {
            identifier
            for identifier, metadata in self.endpoints.items()
            if not metadata.isSource
        }
        ...
        for connection in self.connections:
            ...
            for entity in connection.sinks:
                ...
                entity.assertTrue(
                    ...
                    message="This expression does not match any IO sinks.",
                )
