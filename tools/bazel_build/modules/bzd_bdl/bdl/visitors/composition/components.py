...
class DependencyGroup:
    ...
    def pushGroup(
        self, group: typing.Union["DependencyGroup", typing.List[Expression]]
    ) -> None:
...
class Context:
    ...
    def makeResolver(
        self, symbols: SymbolMap, expression: Expression, **kwargs: typing.Any
    ) -> Resolver:
...
class Components:
    ...
    def resolve(self) -> None:
        ...
        self.resolved = {}
        contexts = {
            context for entries in self.map.values() for context in entries.keys()
        }
        for context in contexts:
            ...
            while entries:
                ...
                if len(entries) == len(remaining):
                    entry = entries[0][1]
                    dependenciesNotMet = [
                        str(e) for e in entry.deps if not isExpressionDiscovered(e)
                    ]
    ...
    def insert(
        self, expression: Expression, entryType: EntryType, context: Context
    ) -> typing.Optional[ExpressionEntry]:
        ...
        if context in entries:
            ...
            def isOverwritable(
                entryTypeFrom: EntryType, entryTypeTo: EntryType
            ) -> bool:
                return (EntryType.service in entryTypeFrom) and (
                    EntryType.workload in entryTypeTo
                )
            ...
            expression.assertTrue(
                ...
                message=f"An expression of role '{entryType}' cannot overwrite an expression of role '{entries[context].entryType}'.",
            )
        ...
        entries[context] = ExpressionEntry(
            expression=expression.deepCopy(), entryType=entryType
        )
        return entries[context]

    def fromIdentifier(
        self, identifier: str, context: Context
    ) -> Result[ExpressionEntry, str]:
        """Get the ExpressionEntry associated with a specific symbol."""

        if identifier not in self.map:
            return Result.makeError(
                f"The expression with the identifier '{identifier}' is not registered in the map."
            )
