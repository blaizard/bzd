...
from bdl.visitors.composition.entities import Entities
...
class CompositionView:
    ...
    def __init__(self, composition: "Composition", target: str) -> None:
        ...
        self.target = target
        self.contexts = {
            context for context in self.composition.contexts if context.target == target
        }
    ...
    def registry(self) -> typing.Dict[Context, typing.Dict[str, ExpressionEntry]]:
        return {
            context: data
            for context, data in self.composition.registry.items()
    ...
    def services(self) -> typing.Dict[Context, typing.List[ExpressionEntry]]:
        return {
            context: data
            for context, data in self.composition.services.items()
            if context in self.contexts
        }
    ...
    def workloads(self) -> typing.Dict[Context, typing.List[ExpressionEntry]]:
        return {
            context: data
            for context, data in self.composition.workloads.items()
            if context in self.contexts
        }
    ...
    def ios(self) -> typing.Dict[Context, typing.List[typing.Dict[str, typing.Any]]]:
        return {
            context: data
            for context, data in self.composition.ios.items()
            if context in self.contexts
        }
...
class Composition:
    ...
    def process(self) -> None:
        ...
        compositionEntities = [
            entity
            for fqn, entity in self.symbols.items(
                groups={Group.composition | Group.topLevel}
            )
    ...
    def __str__(self) -> str:
        ...
        addContent(
            content, "Unique Identifiers", [f"{k}: {v}" for k, v in self.uids.items()]
        )
        ...
        for context in self.contexts:
            addContent(
                ...
                [
                    f"{k}: {v.expression}"
                    for k, v in self.registry.get(context, {}).items()
                ],
            )
            addContent(
                content,
