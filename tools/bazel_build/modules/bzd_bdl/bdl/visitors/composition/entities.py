...
from bdl.visitor import Group
from bdl.visitors.symbol_map import SymbolMap, Resolver
from bdl.entities.impl.entity import Entity
from bdl.entities.impl.expression import Expression
from bdl.entities.builder import ExpressionBuilder
from bdl.entities.impl.types import Category
from bdl.visitors.composition.connections import Connections
from bdl.visitors.composition.components import (
    Components,
    ExpressionEntry,
    EntryType,
    DependencyGroup,
...
class Entities:
        ...
        if expression.isSymbol:
            if expression.symbol.fqn == "bind":
                ...
                for param in expression.parameters:
                    identifiers = set(
                        param.regexpr.match(fqns)
                        if param.isRegexpr
                        else [str(param.symbol)]
                    )
                    for fqn in identifiers:
                        entity = self.symbols.getEntityResolved(fqn=fqn).assertValue(
    ...
    def processMeta(self, expression: Expression) -> None:
        ...
        expression.assertTrue(
            ...
            message="Meta expressions must have a symbol.",
        )
        ...
        for context in self.contexts:
            ...
            for identifier, metadata in self.connections.endpoints.items():
                if metadata.isSource:
                    ...
                    isPartOfContext = bool(
                        self.expressions.fromIdentifier(
                            identifier.this, context=context
                        )
                    )
                    for identifierSink in metadata.connections:
                        if bool(
                            self.expressions.fromIdentifier(
                                identifierSink.this, context=context
                            )
                        ):
                            entry["sinks"].append(
                                {
                                    "history": self.connections.endpoints[
                                        identifierSink
                                    ].history
                                }
                            )
    ...
    def iosRegistry(self) -> typing.Dict[str, typing.Dict[str, typing.Any]]:
        ...
        result: typing.Dict[str, typing.Dict[str, typing.Any]] = {
            fqn: OrderedDict() for fqn in self.registryFQNs
        }
        ...
        for identifier, metadata in self.connections.endpoints.items():
            if identifier.this in result:
                result[identifier.this][identifier.name] = {
                    ...
                    "connections": [
                        str(connection) for connection in metadata.connections
                    ],
                    "symbol": metadata.symbol,
                    "identifier": str(identifier),
                }
    ...
    def getContext(
        self, expression: Expression, defaultExecutor: typing.Optional[str]
    ) -> Context:
            ...
            if executor is not None:
                if expression.symbol.isThis:
                    ...
                    expression.assertTrue(
                        ...
                        message=f"The executors between this expression and its instance, mismatch: '{executor}' vs '{this.executor}'.",
                    )
        ...
        assert executor
        self.symbols.getEntityResolved(fqn=executor).assertValue(
            element=expression.element
        )
    ...
    def process(self, expressions: typing.Iterable[Expression]) -> None:
        ...
        fqns = {
            ...
            if expression.isFQN and expression.isName
        }
        ...
        for expression in expressions:
            ...
            if expression.isSymbol:
                if expression.isExecutor:
                    ...
                    underlyingType = expression.getEntityUnderlyingTypeResolved(
                        resolver=resolver
                    )
        ...
        for expression in executors + workloads:
            context = self.getContext(
                expression=expression, defaultExecutor=defaultExecutor
            )
            ...
            for fqn in expression.dependencies:
                ...
                if isinstance(dependency, Expression):
                    resolver = context.makeResolver(
                        symbols=self.symbols, expression=dependency
                    )
                    dependency.resolveMemoized(resolver=resolver)
                    self.addResolvedDependency(dependency, context=context)

    def addResolved(
        self, expression: Expression, context: Context, isDepedency: bool = False
    ) -> None:
        ...
        if expression.isRoleMeta:
            ...
            self.processEntry(
                expression=expression, isDepedency=isDepedency, context=context
            )
    ...
    def createEntityNestedComposition(
        ...
        entity.resolveMemoized(
            resolver=resolver.make(namespace=resolveNamespace, this=this.fqn)
        )

        return entity

    def processEntry(
        self, expression: Expression, isDepedency: bool, context: Context
    ) -> None:
        ...
        # Register the entry.
        entry = self.expressions.insert(
            expression=expression, entryType=entryType, context=context
        )
        if entry is None:
            return
        ...
        for fqn in expression.dependencies:
            dep = resolver.getEntityResolved(fqn=fqn).assertValue(
                element=expression.element
            )
        ...
        if underlyingType.isInterface:
            ...
            def checkIfInitOrShutdown(
                interfaceEntity: Entity,
            ) -> typing.Optional[DependencyGroup]:
                ...
                if maybeGroup is not None:
                    newEntity = self.createEntityNestedComposition(
                        element=ExpressionBuilder(
                            symbol=f"this.{interfaceEntity.name}"
                        ),
                        this=expression,
        ...
        if underlyingType.isComposition:
            for compositionEntity in underlyingType.composition:
                assert isinstance(compositionEntity, Expression), (
                    "All composition entities must be an expression."
                )
        ...
        if underlyingType.category in {Category.component}:
            result = self.symbols.getChildren(
                fqn=expression.fqn, groups={Group.interface}
            )
