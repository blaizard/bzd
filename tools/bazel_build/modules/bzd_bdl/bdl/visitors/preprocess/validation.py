...
class Validation(VisitorBase[None]):
    ...
    def visitNestedEntities(self, entity: Nested, result: None) -> None:
        ...
        if entity.category == Category.interface:
            entity.assertTrue(
                condition=entity.isName, message="Interfaces must have a valid name."
            )
            entity.assertTrue(
                condition=interfaceCategories.issubset(
                    {Category.method, Category.expression}
                ),
                message="Interfaces can only expose methods and expressions as interface.",
            )
            entity.assertTrue(
                condition=configCategories.issubset(
                    {Category.expression, Category.using}
                ),
            ...
            entity.assertTrue(
                condition=entity.isName, message="Structures must have a valid name."
            )
            entity.assertTrue(
                condition=interfaceCategories.issubset(
                    {
            ...
            entity.assertTrue(
                condition=entity.isName, message="Components must have a valid name."
            )
            entity.assertTrue(
                condition=interfaceCategories.issubset(
                    {Category.method, Category.expression}
            ...
            entity.assertTrue(
                condition=compositionCategories.issubset({Category.expression}),
                message="Components composition can only contain expressions.",
            )

        elif entity.category == Category.composition:
            entity.assertTrue(
                condition=not entity.hasInheritance,
                message="Compositions cannot have inheritance.",
            )
            entity.assertTrue(
                condition=interfaceCategories.issubset({Category.expression}),
            ...
            entity.assertTrue(
                condition=False,
                message=f"Unsupported nested category: '{entity.category}'.",
            )
    ...
    def visitMethod(self, entity: Method, result: None) -> None:
        ...
        for arg in entity.parameters:
            ...
            arg.assertTrue(
                condition=(arg.name not in argumentNames),
