...
_regexprPreset = (
    r"\{(?P<preset>" + _regexprBaseName + r"(?:\." + _regexprBaseName + r")*)\}"
)
...
def _makeRegexprFQN(name: str) -> str:
    return (
        r"(?P<" + name + r">" + _regexprBaseName + r"(?:\." + _regexprBaseName + r")*)"
    )
...
class FragmentBlockComment(FragmentComment):
    def process(self, match: typing.Match[str]) -> None:
        ...
        self.attrs["comment"] = re.sub(
            re.compile(r"^(\s*\*)+", re.MULTILINE), "", self.attrs["comment"]
        )
...
def makeGrammarNested(
    ...
    grammarAfterName: Grammar = [
        ...
        GrammarItem(
            ...
            NestedInterface,
            nestedGrammar
            + [makeNestedCategory(nestedName) for nestedName in nested]
            + [
...
def makeGrammarSymbol(nextGrammar: Grammar) -> Grammar:
    ...
    def makeGrammar(
        continuation: Grammar, nested: typing.Optional[Grammar] = None
    ) -> Grammar:
        ...
        grammar.append(
            GrammarItem(
                ...
                [
                    GrammarItem(
                        r"<", TemplateStart, [grammar if nested is None else nested]
                    ),
                    continuation,
                ],
            )
...
def makeGrammarExpressionFragment(
    ...
    def makeGrammar(
        ...
        grammarValue.append(
            GrammarItem(_regexprRegexpr, FragmentNewElement, grammarValue)
        )

        # Test for operator.
        grammarValue.append(
            GrammarItem(_regexprOperator, FragmentNewElement, grammarValue)
        )

        # Test for value.
        grammarValue.append(
            GrammarItem(_regexprValue, FragmentNewElement, grammarValue)
        )

        # Test for preset.
        grammarValue.append(
            GrammarItem(_regexprPreset, FragmentNewElement, grammarValue)
        )
    ...
    nestedGrammar = makeGrammar(
        [
            GrammarItem(",", FragmentNewElement),
            GrammarItem(r"\)", FragmentParentElement),
        ]
    )
...
class Parser(ParserBase):
    def __init__(self, content: str) -> None:
        ...
        super().__init__(
            ...
            + makeGrammarNested(
                withinNested, name="interface", defaultNested="interface"
            )
            + makeGrammarNested(
                withinNested, name="composition", defaultNested="composition"
            )
