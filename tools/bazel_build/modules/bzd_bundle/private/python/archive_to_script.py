...
def tarCompressionArgument(compression: str) -> str:
    ...
    assert compression in compressionToArg, (
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "args",
        nargs="*",
        help="The rest of the arguments to be passed to the entry point.",
    )
...
""".format(
                    ...
                    stamp=stamp,
                    compression=tarCompressionArgument(args.compression)
                    if args.compression
                    else "",
                    args=" ".join(args.args),
                ).encode("utf-8")
