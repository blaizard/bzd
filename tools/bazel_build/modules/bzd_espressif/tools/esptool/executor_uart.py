...
def getDevice(
    vidPids: typing.List[typing.Tuple[int, int]], device: typing.Optional[Device]
) -> Device:
    ...
    if not devices.empty():
        ...
        if not filteredDevices.empty():
            ...
            if accessibleDevices.empty():
                ...
                logger.error(
                    "Use the following command and reboot: sudo usermod -a -G dialout $USER"
                )
                sys.exit(1)
            return accessibleDevices.get()[0]

        logger.info("No device compatible, please use one from the following list:")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--baud_rate", type=int, default=115200, help="Baudrate of the communication."
    )
    ...
    parser.add_argument(
        "--stop_bits", type=float, default=1, help="Number of stop bits."
    )
    parser.add_argument(
        "--flow_control",
    ...
    assert pathlib.Path(args.esptool).is_file(), (
        f"Invalid path for esptool: {args.esptool}"
    )
    ...
    if result.getReturnCode() != 0:
        ...
        logger.info(
            "- Connection failed `Timed out waiting for packet header`: try to press the BOOT button."
        )
        sys.exit(1)
