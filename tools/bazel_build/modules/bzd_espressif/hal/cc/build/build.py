...
def copyFileTree(
    output: pathlib.Path, paths: typing.Iterable[pathlib.Path], pattern: str
) -> None:
...
if __name__ == "__main__":
    ...
    assert "IDF_PATH" in os.environ, (
        "The ESP-IDF must be properly installed, make sure you run esp-idf/export.sh"
    )
    assert "BUILD_WORKSPACE_DIRECTORY" in os.environ, (
        "This script must be run with bazel."
    )
    ...
    localCommand(
        cmds=["cmake", "..", f"-DIDF_TARGET={args.target}", "-G", "Unix Makefiles"],
        cwd=buildPath,
        stdout=True,
        stderr=True,
        env=env,
    ...
    localCommand(
        cmds=["make"], cwd=buildPath, stdout=True, stderr=True, env=env, timeoutS=300
    )
    ...
    for entry in compile_commands:
        if pathlib.Path(entry["file"]) == project / "main/main.cc":
            assert not commands["compile"].isDiscovered, (
                "The compile command has been discovered twice."
            )
    ...
    commands["link"].command = (
        project / "build/CMakeFiles/bzd.elf.dir/link.txt"
    ).read_text()
