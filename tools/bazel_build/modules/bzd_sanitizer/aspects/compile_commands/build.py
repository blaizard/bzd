...
class CompileCommands:
    ...
    def insert(self, fragment: typing.Dict[str, str]) -> None:
        ...
        index = next(
            (i for i, x in enumerate(self.compile_commands) if file == x["file"]), -1
        )
        if index == -1:
...
class Manager:
    def __init__(
        self, workspace_path: pathlib.Path, compile_commands_path: pathlib.Path
    ) -> None:
                    ...
                    while self.queue.notEmpty():
                        ...
                        try:
                            ...
                            fragments = json.loads(
                                (self.workspace_path / f).read_text()
                            )
                        except Exception:
                            continue
...
if __name__ == "__main__":
    ...
    files = [
        pathlib.Path(f.strip()) for f in result.getOutput().split("\n") if f.strip()
    ]

    manager = Manager(
        workspace_path=workspace_path, compile_commands_path=compile_commands_path
    )
