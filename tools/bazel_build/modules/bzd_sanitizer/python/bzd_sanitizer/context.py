...
class Context:
    ...
    def _excludeFile(
        ...
        while searchDir != pathlib.Path("."):
            ...
            if ignoreFilePath not in cache:
                ...
                cache[ignoreFilePath] = (
                    Filter.fromFile(ignoreFilePathFullPath)
                    if ignoreFilePathFullPath.is_file()
                    else None
                )
    ...
    def data(
        ...
        for path in self.fileList:
            ...
            isIncluded = isIncluded or bool(
                endswith and path.lower().endswith(endswith)
            )
    ...
    def printAction(self, action: str) -> _ContextPrintAction:
        ...
        return _ContextPrintAction(
            self, self.lineWidth - len(lineString) + self.colorSize()
        )
