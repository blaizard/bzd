...
def chunkWorker(
    contextPath: pathlib.Path,
    workload: typing.Callable[
        [
            typing.Tuple[pathlib.Path, typing.Sequence[pathlib.Path], bool, *Ts],
            typing.TextIO,
        ],
        bool,
    ],
    ...
    timeoutS: int = 60,
    maxFiles: int = 256,
    stdoutParser: typing.Optional[
        typing.Callable[[str, pathlib.Path], typing.Iterator[OutputWithPath]]
    ] = None,
    **filters: typing.Any,
) -> bool:
    ...
    for i in range(0, len(queue), maxFiles):
        ...
        stdout = io.StringIO()
        if not workload(
            [context.workspace, paths, context.check] + list(args or []),  # type: ignore
            stdout,
        ):
            isFailure = True
            stdout.seek(0)
    ...
    if isFailure:
        if stdoutParser:
            ...
            for path, outputs in sorted(outputsByPath.items(), key=lambda x: x[0]):
                try:
                    actualPath = path.resolve(strict=True).relative_to(
                        context.workspace
                    )
...
def worker(
    contextPath: pathlib.Path,
    workload: typing.Callable[
        [typing.Tuple[pathlib.Path, pathlib.Path, bool, *Ts], typing.TextIO], bool
    ],
    args: typing.Optional[typing.Tuple[*Ts]] = None,
    timeoutS: int = 60,
    **filters: typing.Any,
