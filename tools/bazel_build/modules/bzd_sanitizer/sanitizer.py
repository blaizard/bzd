...
def getFileListFromResult(
    result: typing.Any, workspace: pathlib.Path
) -> typing.List[str]:
    """Get a result and returns a list of existing files."""

    return list(
        {f for f in result.getStdout().split("\n") if (workspace / f).is_file()}
    )


def getFileListFromRecursiveGit(
    workspace: pathlib.Path, path: pathlib.Path, command: typing.List[str]
) -> typing.Sequence[str]:
    ...
    result = localCommand(
        [
            ...
            "--quiet",
            "foreach",
            " ".join(command) + ' | sed "s|^|$path/|"',
        ],
        cwd=workspace,
    )
    output += [
        f
        for f in getFileListFromResult(result, workspace=workspace)
...
def getFileList(
    workspace: pathlib.Path, path: pathlib.Path, all: bool
) -> typing.Sequence[str]:
    ...
    if all:
        ...
        fileList = getFileListFromRecursiveGit(
            workspace=workspace,
            path=path,
            command=["git", "ls-files", "--other", "--modified", "--exclude-standard"],
        )

    # If there are no files, try a diff with the last commit.
    if len(fileList) == 0:
        result = localCommand(
            ["git", "diff", "--name-only", "--diff-filter=ACMRU", "HEAD~1", str(path)],
            cwd=workspace,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-u", "--use", type=str, action="append", default=[], help="Actions to be used."
    )
    parser.add_argument(
        "-a", "--all", action="store_true", help="Sanitize all files in the workspace."
    )
    parser.add_argument(
        "-c",
        "--check",
    ...
    parser.add_argument(
        "path", type=pathlib.Path, default=".", nargs="?", help="The path to sanitize."
    )
    ...
    try:
        ...
        for action in args.use:
            ...
            if not passed:
                ...
                print(
                    f"{context.color('command')}bazel run {action} -- {contextFilePath}{context.color()}"
                )
                success = False
