    ...
    def renderTemplate(self, templatePath: pathlib.Path) -> str:
        ...
        for stage in rawData["stages"]:
            ...
            assert isinstance(item.get("name"), str), (
                "Missing 'name' attribute, must be a string."
            )
            assert isinstance(item.get("commands"), list), (
                "Missing 'commands' attribute, must be a list."
            )
            if item["category"] in configs:
                ...
                item["commands"] = [
                    line.format(**asdict(config)) for line in item["commands"]
                ]
                data.append(item)

        template = Template.fromPath(templatePath)
        return template.render(
            {"stages": data, "normalizeIdentifier": Factory.normalizeIdentifier}
        )
