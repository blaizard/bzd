...
class Sh(Factory):
    ...
    def isCompatible(self) -> bool:
        ...
        return os.environ.get("SHELL", "").endswith(("/sh", "/bash",))

    def build(self, workspace: pathlib.Path) -> None:
        """Build the configuration if needed."""

        content = self.renderTemplate(pathlib.Path("tools/shell/sh/srcs/bashrc.btl"), {"nonInteractive": pathlib.Path("tools/shell/sh/srcs/non_interactive"), "interactive": pathlib.Path("tools/shell/sh/srcs/interactive")})
