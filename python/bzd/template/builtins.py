...
class BuiltinsBtl:
    ...
    def merge(*args: typing.Any) -> typing.Any:
        ...
        assert len(args) > 1, (
            "The builtin function 'merge' requires at least 2 arguments."
        )
        first = args[0]
        assert all(isinstance(i, type(first)) for i in args[1:]), (
            "The builtin function 'merge' requires all its elements to be of identical types."
        )
        ...
        assert False, (
            f"The builtin function 'merge' cannot take arguments of type '{type(first)}'."
        )
