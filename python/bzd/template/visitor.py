...
class Visitor(VisitorBase[ResultType, ResultType]):
    ...
    def __init__(
        ...
        self.substitutions = (
            substitutions
            if isinstance(substitutions, SubstitutionWrapper)
            else SubstitutionWrapper(substitutions)
        )
    ...
    def resolveExpressions(self, sequence: Sequence) -> typing.List[typing.Any]:
        ...
        operators: typing.Dict[
            str, typing.Callable[[typing.Any, typing.Any], typing.Any]
        ] = {
        ...
        operatorsUnary: typing.Dict[str, typing.Callable[[typing.Any], typing.Any]] = {
            ...
            try:
                ...
                if elementType in operators:
                    assert operator is None, (
                        f"Detected 2 consecutive operators '{operator}' and '{elementType}'."
                    )
                    operator = elementType

                elif elementType in operatorsUnary:
                    assert operatorUnary is None, (
                        f"Detected 2 consecutive unary operators '{operatorUnary}' and '{elementType}'."
                    )
                    ...
                    if elementType == "symbol":
                        ...
                        for symbol in element.getNestedSequenceAssert(kind="symbol"):
                            symbolType = (
                                symbol.getAttr("type").value
                                if symbol.isAttr("type")
                                else "symbol"
                            )
                            if symbolType == "symbol":
                                ...
                                assert callable(value), "The value is not callable."
                                args = self.resolveExpressions(
                                    symbol.getNestedSequenceAssert("callable")
                                )
                                value = value(*args)
                            elif symbolType == "array":
                                key = self.resolveExpression(
                                    symbol.getNestedSequenceAssert("array")
                                )
                                value = substitute(value, key)
                            else:
                    ...
                    if operator is not None:
                        assert len(values) > 0, (
                            "There is no value to apply this {operator} operator."
                        )
    ...
    def resolveExpression(self, sequence: Sequence) -> typing.Any:
        ...
        assert len(values) == 1, (
            f"There are more than a single value: {[type(v) for v in values]}."
        )
        return values[0]
    ...
    def visitSubstitution(self, element: Element, result: ResultType) -> None:
        ...
        # Save the output
        assert isinstance(value, (int, float, str, pathlib.Path)), (
    ...
    def appendSubstitution(
        self, element: Element, result: ResultType, string: str
    ) -> ResultType:
    ...
    def appendBlock(
        self, element: Element, result: ResultType, block: ResultType
    ) -> ResultType:
        result += block
        return result
    ...
    def visitForBlock(self, element: Element) -> ResultType:
        ...
        iterable = self.resolveExpression(
            sequence=element.getNestedSequenceAssert("iterable")
        )
        loop = Loop(
            self.substitutions["loop"] if "loop" in self.substitutions else None
        )
    ...
    def visitMacro(self, element: Element) -> None:
        ...
        Error.assertTrue(
            ...
            message="Name conflict with macro and an already existing name: '{}'.".format(
                name
            ),
        )
    ...
    def visitInclude(self, element: Element) -> ResultType:
        ...
        Error.assertTrue(
            ...
            message="The include path must resolve into a string, instead: '{}'.".format(
                includePathStr
            ),
        )
        ...
        Error.assertTrue(
            ...
            message="Unable to find '{}' within {}".format(
                includePathStr, str([f.as_posix() for f in self.includeDirs])
            ),
        )
