...
def makeGrammarExpression(
    name: str, continuation: Grammar, attrs: typing.Dict[str, str] = {}
) -> Grammar:
    ...
    return [
        GrammarItem(
            None,
            ExpressionStart,
            makeGrammarExpressionImpl(
                [GrammarItem(None, FragmentParentElement, continuation)]
            ),
        )
    ]
...
def makeGrammarExpressionImpl(continuation: Grammar) -> Grammar:
    ...
    symbol.extend(
        [
            GrammarItemSpaces,
            GrammarItem(
                makeRegexprSymbol("value"), FragmentNewElement, symbolTransition
            ),
        ]
    )
    ...
    return [
        ...
        GrammarItem(
            _regexprBooleanFalse, makeFragmentNewElement("false"), finalGrammar
        ),
            ...
            GrammarItem(
                ...
                [
                    makeGrammarExpression(
                        "value", [makeGrammarSubstitutionStop(FragmentNewElement)]
                    )
                ],
            )
        ]
...
def makeGrammarControlFor() -> Grammar:
    ...
    grammarFromIn = [
        GrammarItem(
            ...
            Fragment,
            makeGrammarExpression(
                "iterable", makeGrammarControlStop(FragmentNestedStart, "root")
            ),
        )
    ]
    ...
    return makeGrammarControlStart(
        ...
        [
            GrammarItem(
                ...
                {"category": "if"},
                makeGrammarExpression(
                    "condition", makeGrammarControlStop(FragmentNestedStart, "root")
                ),
            )
        ],
    )
...
def makeGrammarControlElseIf() -> Grammar:
    ...
    return [
        GrammarItem(
            ...
            + makeGrammarControlStart(
                r"else", makeGrammarControlStop(ElseFragment, "root")
            ),
        )
    ]
...
def makeGrammarControl() -> Grammar:
    ...
    return [
        GrammarItem(
            r"(?={%)",
            Fragment,
            makeGrammarControlFor()
            + makeGrammarControlIf()
            + makeGrammarControlElseIf()
            + makeGrammarControlMacro()
            + makeGrammarControlInclude()
...
class Parser(ParserBase):
    def __init__(self, content: str) -> None:
        super().__init__(
            content,
            grammar=makeGrammarEscape()
            + makeGrammarSubstitution()
            + makeGrammarControl()
            + makeGrammarComments()
            + makeGrammarContent(),
