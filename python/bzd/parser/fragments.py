...
class Fragment:
    ...
    def next(
        self, element: "ElementParser", grammar: Optional["Grammar"]
    ) -> "ElementParser":
...
class FragmentComment(Fragment):
    ...
    def merge(self, attrs: Attributes) -> None:
        for key, value in self.attrs.items():
            ...
            minTrailingSpaces = min(
                [len(s) - len(s.lstrip(" ")) for s in value.split("\n")]
            )
            updatedValue = "\n".join(
                [s[minTrailingSpaces:].rstrip() for s in value.split("\n")]
            )
            ...
            if key in attrs:
                ...
                attrs[key] = AttributeParser(
                    index=IGNORE_INDEX_VALUE, end=0, value=updatedValue
                )
