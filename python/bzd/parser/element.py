...
ElementSerialize = typing.MutableMapping[
    str, typing.Union[AttributesSerialize, typing.List[typing.Any]]
]
...
class Sequence:
    ...
    def __add__(
        self, other: typing.Union[typing.List["Element"], "Sequence"]
    ) -> "Sequence":
    ...
    @staticmethod
    def fromSerialize(
        sequence: SequenceSerialize, context: typing.Optional[Context] = None
    ) -> "Sequence":
...
class Element:
    ...
    @staticmethod
    def fromSerialize(
        element: ElementSerialize, context: typing.Optional[Context] = None
    ) -> "Element":
        ...
        e.attrs = {
            key: Attribute.fromSerialize(attr) for key, attr in element["@"].items()
        }
        e.sequences = {
            key: Sequence.fromSerialize(sequence, Context(parent=e))  # type: ignore
            for key, sequence in element.items()
            if key != "@"
        }
        return e
    ...
    def getAttr(self, name: str) -> Attribute:
        ...
        assert name in self.attrs, (
            f"Attribute '{name}' is not present in '{str(self)}'."
        )
    ...
    def makeContext(
        self, attr: typing.Optional[str] = None
    ...
    def __eq__(self, other: object) -> bool:
        if not isinstance(other, Element):
            return False
        return isEqual(
            self.serialize(ignoreContext=False), other.serialize(ignoreContext=False)
        )
    ...
    def toString(self, nested: bool = True, simplified: bool = False) -> str:
        ...
        attributeFormat = (
            '{key}="{value}"' if simplified else '{key}:{index}:{end}="{value}"'
        )
        content = "<Element {}/>".format(
            " ".join(
                [
                    attributeFormat.format(
                        key=key, index=attr.index, end=attr.end, value=attr.value
                    )
...
class ElementBuilder(Element):
    ...
    def setAttr(
        self: U, key: str, value: str, index: int = IGNORE_INDEX_VALUE, end: int = 0
    ) -> U:
    ...
    def pushBackElementToNestedSequence(self: U, kind: str, element: Element) -> U:
        ...
        SequenceBuilder.cast(self.sequences[kind], SequenceBuilder).pushBackElement(
            element
        )
        return self
    ...
    def pushFrontElementToNestedSequence(self: U, kind: str, element: Element) -> U:
        ...
        if kind not in self.sequences:
            self.sequences[kind] = Sequence(context=Context(parent=self))
        SequenceBuilder.cast(self.sequences[kind], SequenceBuilder).pushFrontElement(
            element
        )
        return self
    ...
    def removeElementFromNestedSequence(self: U, kind: str, element: Element) -> U:
        ...
        if kind in self.sequences:
            SequenceBuilder.cast(self.sequences[kind], SequenceBuilder).removeElement(
                element
            )
        return self
...
class ElementParser(Element):
    ...
    def getSequence(self) -> SequenceParser:
        ...
        assert isinstance(self.parent, SequenceParser), (
            "parent must be a sequence, instead {}".format(type(self.parent))
        )
        return self.parent
