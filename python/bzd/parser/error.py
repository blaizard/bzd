...
class AssertionResult:
    ...
    def extend(
        ...
        if bool(self):
            self.message += Error.toStringFromElement(
                element=element, attr=attr, message=message
            )  # type: ignore
            if throw:
...
class Error:
    ...
    @staticmethod
    def toString(
        context: typing.Optional[Context], index: int, end: int, message: str
    ) -> str:
        ...
        if context.content is None:
            ...
            try:
                ...
                logging.warning(
                    "Could not open file '{}'.".format(context.path.as_posix())
                )
                return message
        else:
        ...
        contentByLine.insert(
            line + 1, "{padding}^".format(padding=paddingLine[0:column])
        )
        contentByLine.insert(
            line + 2,
    ...
    @staticmethod
    def toStringFromElement(
        element: Element, attr: typing.Optional[str] = None, message: str = "Error"
    ) -> str:
    ...
    @staticmethod
    def handle(
        context: Context, index: int, end: int, message: str, throw: bool = True
    ) -> str:
    ...
    def assertTrue(
        ...
        if not condition:
            return Error.handleFromElement(
                element=element, attr=attr, message=message, throw=throw
            )
        return AssertionResult()

    @staticmethod
    def assertHasAttr(
        element: Element, attr: str, throw: bool = True
    ) -> AssertionResult:
        """
    ...
    @staticmethod
    def assertHasSequence(
        element: Element, sequence: str, throw: bool = True
    ) -> AssertionResult:
        """
