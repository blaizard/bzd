...
from bzd.parser.grammar import Grammar, GrammarItem
...
class Parser:
    ...
    @classmethod
    def fromPath(
        cls: Type["Parser"], path: Path, *args: Any, **kwargs: Any
    ) -> "Parser":
        """
    ...
    def iterateGrammar(self, grammar: Grammar) -> Iterator[GrammarItem]:
        ...
        for item in grammar:
            if isinstance(item, list):
                ...
                assert False, (
                    "Grammar item must be of type Grammar or GrammarItem, received: {}".format(
                        type(item)
                    )
                )

    def getGrammar(
        self, checkpoints: MutableMapping[str, Grammar], item: GrammarItem
    ) -> Optional[Grammar]:
        """
        ...
        if isinstance(item.grammar, str):
            ...
            assert item.checkpoint is None, (
                "A grammar item referencing to a checkpoint cannot set a checkpoint."
            )
    ...
    def parse(self) -> SequenceParser:
        ...
        try:
            while index < len(content):
                m = None
                for item in self.iterateGrammar(
                    self.defaultGrammarPre
                    + element.getGrammar()
            ...
            print("**** debug ****\n", root)
            for item in self.iterateGrammar(
                self.defaultGrammarPre + element.getGrammar() + self.defaultGrammarPost
            ):
                print(item.regexpr)
