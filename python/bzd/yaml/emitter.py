...
class Emitter:
	...
	def check_simple_key(self):
		...
		return length < 128 and (isinstance(self.event, AliasEvent) or
		                         (isinstance(self.event, ScalarEvent) and not self.analysis.empty
		                          and not self.analysis.multiline) or self.check_empty_sequence()
	...
	def write_single_quoted(self, text, split=True):
		...
		while end <= len(text):
			...
			if spaces:
				if ch is None or ch != " ":
					if (start + 1 == end and self.column > self.best_width and split and start != 0
					    and end != len(text)):
						self.write_indent()
					else:
						data = text[start:end]
	...
	def write_double_quoted(self, text, split=True):
		...
		while end <= len(text):
			...
			if (ch is None or ch in '"\\\x85\u2028\u2029\uFEFF'
			    or not ("\x20" <= ch <= "\x7E" or (self.allow_unicode and
			                                       ("\xA0" <= ch <= "\uD7FF" or "\uE000" <= ch <= "\uFFFD")))):
				if start < end:
					data = text[start:end]
					self.column += len(data)
