...
class Scanner:
	...
	def fetch_more_tokens(self):
		...
		raise ScannerError("while scanning for the next token", None,
		                   "found character %r that cannot start any token" % ch, self.get_mark())
	...
	def stale_possible_simple_keys(self):
		...
		for level in list(self.possible_simple_keys):
			...
			if key.line != self.line or self.index - key.index > 1024:
				if key.required:
					raise ScannerError("while scanning a simple key", key.mark, "could not find expected ':'",
					                   self.get_mark())
	...
	def scan_yaml_directive_value(self, start_mark):
		...
		if self.peek() != '.':
			raise ScannerError("while scanning a directive", start_mark,
			                   "expected a digit or '.', but found %r" % self.peek(), self.get_mark())
		self.forward()
	...
	def scan_yaml_directive_number(self, start_mark):
		...
		ch = self.peek()
		if not ('0' <= ch <= '9'):
			raise ScannerError("while scanning a directive", start_mark, "expected a digit, but found %r" % ch,
			                   self.get_mark())
		length = 0
	...
	def scan_tag_directive_handle(self, start_mark):
		...
		ch = self.peek()
		if ch != ' ':
			raise ScannerError("while scanning a directive", start_mark, "expected ' ', but found %r" % ch,
			                   self.get_mark())
		return value
	...
	def scan_directive_ignored_line(self, start_mark):
		...
		if ch not in '\0\r\n\x85\u2028\u2029':
			raise ScannerError("while scanning a directive", start_mark,
			                   "expected a comment or a line break, but found %r" % ch, self.get_mark())
	...
	def scan_tag(self):
		...
		if ch == '<':
			...
			if self.peek() != '>':
				raise ScannerError("while parsing a tag", start_mark, "expected '>', but found %r" % self.peek(),
				                   self.get_mark())
			self.forward()
		elif ch in '\0 \t\r\n\x85\u2028\u2029':
			handle = None
	...
	def scan_flow_scalar_spaces(self, double, start_mark):
		...
		ch = self.peek()
		if ch == '\0':
			raise ScannerError("while scanning a quoted scalar", start_mark, "found unexpected end of stream",
			                   self.get_mark())
		elif ch in '\r\n\x85\u2028\u2029':
			line_break = self.scan_line_break()
	...
	def scan_flow_scalar_breaks(self, double, start_mark):
		...
		while True:
			...
			if (prefix == '---' or prefix == '...') and self.peek(3) in '\0 \t\r\n\x85\u2028\u2029':
				raise ScannerError("while scanning a quoted scalar", start_mark, "found unexpected document separator",
				                   self.get_mark())
			while self.peek() in ' \t':
				self.forward()
			if self.peek() in '\r\n\x85\u2028\u2029':
	...
	def scan_plain(self):
		...
		while True:
			...
			while True:
				ch = self.peek(length)
				if ch in '\0 \t\r\n\x85\u2028\u2029' or (
				    ch == ':' and self.peek(length + 1) in '\0 \t\r\n\x85\u2028\u2029' +
				    (u',[]{}' if self.flow_level else u'')) or (self.flow_level and ch in ',?[]{}'):
					break
				length += 1
			if length == 0:
				break
	...
	def scan_tag_handle(self, name, start_mark):
		...
		ch = self.peek()
		if ch != '!':
			raise ScannerError("while scanning a %s" % name, start_mark, "expected '!', but found %r" % ch,
			                   self.get_mark())
		length = 1
		ch = self.peek(length)
		if ch != ' ':
	...
	def scan_tag_uri(self, name, start_mark):
		...
		if length:
			...
			length = 0
		if not chunks:
			raise ScannerError("while parsing a %s" % name, start_mark, "expected URI, but found %r" % ch,
			                   self.get_mark())
		return ''.join(chunks)
