...
class Parser:
    ...
    def setParsedData(
        ...
        if sections:
            ...
            self.mapping = {
                section: Parser.unsetAggregrate for section in self.sections
            }
        if units:
    ...
    def _aggregateSections(sections: Fragment) -> SectionType:
        ...
        return {
            section: obj
            for section, obj in aggregatedSections.items()
            if obj["size"] > 0
        }
    ...
    def _aggregateUnits(units: Fragment, areUnitsPath: bool) -> UnitsType:
        ...
        for unit, sections in aggregatedUnits.items():
            aggregatedUnits[unit] = {
                section: size for section, size in sections.items() if size > 0
            }
        # Normalize units if it represents path
        if areUnitsPath:
            return {
                os.path.normpath(key): value for key, value in aggregatedUnits.items()
            }
    ...
    def filter(self, pattern: str) -> None:
        ...
        self.sections = {
            ...
            if not regexpr.fullmatch(section)
        }
        # Filter units
