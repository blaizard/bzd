...
from bzd.apps.map_analyzer.parser.parser import Parser
...
) -> typing.Optional[typing.Tuple[FilterConfigType, AggregateConfigType]]:
    """
...
def analyze(data: Parser) -> None:
    ...
    sortedByAddresses: typing.List[typing.Dict[str, typing.Any]] = sorted(
        [obj for address, obj in byAddresses.items()], key=lambda x: x["address"]
    )
    ...
    for item in sortedByAddresses:
        ...
        if address < addressEnd:
            print(
                "Section '{}'@{:#08x} overlaps with previous section '{}'.".format(
                    section, address, previousSection
                ),
                file=sys.stderr,
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-o", "--output", default="map.json", help="Path of the json output."
    )
    parser.add_argument(
        "-b",
        "--binary",
    ...
    parser.add_argument(
        "map", type=pathlib.Path, help="Path of the map file to analyze."
    )
