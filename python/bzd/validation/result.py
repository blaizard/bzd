...
from bzd.validation.schema import ProcessedResult, Values
...
class Result(typing.Generic[T]):
    ...
    def __repr__(self) -> str:
        ...
        if self.isList:
            ...
            FORMAT_NOT_VALIDATE_MULTI = (
                "Some positional values do not validate: {message}"
            )
        else:
        ...
        if len(self.errors.keys()) == 1:
            for key, result in self.errors.items():
                content.append(
                    FORMAT_NOT_VALIDATE_SINGLE.format(
                        key=key, message=", ".join(result)
                    )
                )
        else:
            messages = [
                '{}: "{}"'.format(key, ", ".join(result))
                for key, result in self.errors.items()
            ]
            content.append(
                FORMAT_NOT_VALIDATE_MULTI.format(message="; ".join(messages))
            )
