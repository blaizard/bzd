...
class Min(Constraint):
    def install(self, processedSchema: ProcessedSchema, args: typing.List[str]) -> None:
        ...
        def _process(arg: int, context: Context) -> None:
            assert len(context.value) >= arg, (
                f"the string '{context.value}' is shorter than its required minimum size: {arg}"
            )
...
class Max(Constraint):
    def install(self, processedSchema: ProcessedSchema, args: typing.List[str]) -> None:
        ...
        def _process(arg: int, context: Context) -> None:
            assert len(context.value) <= arg, (
                f"the string '{context.value}' is longer than its required maximum size: {arg}"
            )
