...
class Min(Constraint):
    def install(self, processedSchema: ProcessedSchema, args: typing.List[str]) -> None:
        ...
        def _process(arg: float, context: Context) -> None:
            assert context.underlying >= arg, (
                f"the value '{context.underlying}' is lower than the required minimum: {arg}"
            )
...
class Max(Constraint):
    def install(self, processedSchema: ProcessedSchema, args: typing.List[str]) -> None:
        ...
        def _process(arg: float, context: Context) -> None:
            assert context.underlying <= arg, (
                f"the value '{context.underlying}' is higher than the required maximum: {arg}"
            )
