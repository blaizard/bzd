...
from bzd.http.client import HttpClient, HttpResponse


class HttpResponseMock(HttpResponse):
    def __init__(
        self, status: int, content: bytes, headers: typing.Dict[str, str]
    ) -> None:
        self._status = status
...
class HttpClientMock:
    ...
    def _request(
        ...
        response = self.callback(
            method=method, url=url, body=body, headers=headers, timeoutS=timeoutS
        )
        return HttpClientMock.makeResponse(response)

    @staticmethod
    def makeResponse(
        response: typing.Optional[typing.Union[str, bytes, HttpResponse]],
    ) -> HttpResponseMock:
        status = 200
        content = b""
        headers: typing.Dict[str, str] = {}
