...
class HttpParser:
    ...
    @staticmethod
    def getHeader(
        data: bytes, name: str, defaultValue: typing.Any = None
    ) -> typing.Any:
    ...
    def _readHeaders(self) -> None:
        ...
        assert self.transferMode is not None, "Unsupported transfer mode."

        # Read the payload length.
        if self.transferMode == TransferMode.normal:
            contentLength = self.getHeader(header, "content-length")
            assert contentLength is not None, "Content-length header is missing."
    ...
    def read(self) -> typing.Generator[bytes, None, None]:
        ...
        if self.transferMode == TransferMode.normal:
            assert self.bodyLengthLeft is not None, (
                "Body length information is missing."
            )
