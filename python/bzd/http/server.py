    ...
    def _handleStatic(self) -> bool:
        ...
        for uri, path in self.staticRoutes.items():
            if self.path.lower().startswith(uri):
                ...
                self.send_header(
                    "Content-type", mimetypes.guess_type(fullPath)[0] or "text/html"
                )
                self.end_headers()
...
class HttpServer(socketserver.TCPServer):
    ...
    def __init__(self, port: int = 0, bind: str = "0.0.0.0") -> None:
        ...
        self.staticRoutes: StaticRoutes = {}
        super().__init__(
            (bind, port), partial(Handler, self.handlers, self.staticRoutes)
        )
    ...
    def addRoute(self, method: str, uri: str, handler: HandlerCallback) -> None:
        method = method.lower()
        assert method in self.handlers, (
            f"The route method {method} is not valid, expected any of {str(self.handlers.keys())}."
        )
        assert uri not in self.handlers[method], (
            f"The route '{uri}' is already set for '{method}'."
        )
