...
class TestRun(unittest.TestCase):
    def testUpdateDeepNoConflict(self) -> None:
        ...
        output = updateDeep(
            {"a": {"b": 2}}, {"b": 2}, policy=UpdatePolicy.raiseOnConflict
        )
        self.assertEqual(output, {"a": {"b": 2}, "b": 2})
        output = updateDeep(
            {"b": 2}, {"a": {"b": 2}}, policy=UpdatePolicy.raiseOnConflict
        )
        self.assertEqual(output, {"a": {"b": 2}, "b": 2})
        ...
        output = updateDeep(
            {"a": {"b": 2}}, {"a": {"c": 3}}, policy=UpdatePolicy.raiseOnConflict
        )
        self.assertEqual(output, {"a": {"b": 2, "c": 3}})
        output = updateDeep(
            {"a": {"c": 3}}, {"a": {"b": 2}}, policy=UpdatePolicy.raiseOnConflict
        )
        self.assertEqual(output, {"a": {"b": 2, "c": 3}})
    ...
    def testUpdateDeepRaiseNonConflict(self) -> None:
        ...
        output = updateDeep(
            {"a": 1}, {"a": {"c": 2}}, policy=UpdatePolicy.raiseOnNonConflict
        )
