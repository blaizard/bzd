...
class Devices:
    ...
    def __init__(
        self, devices: typing.Optional[typing.List[typing.Any]] = None
    ) -> None:
        self.devices = (
            sorted(serial.tools.list_ports.comports()) if devices is None else devices  # type: ignore
        )

    def filterByVidPids(
        self, vidPids: typing.Sequence[typing.Tuple[int, int]]
    ) -> "Devices":
    ...
    def filterByAccess(self) -> "Devices":
        ...
        return Devices(
            [info for info in self.devices if os.access(info.device, os.R_OK | os.W_OK)]
        )
    ...
    def getInfo(self) -> typing.Dict[Device, str]:
        ...
        return {
            info.device: "{}, {}".format(info.description, info.hwid)
            for info in self.devices
        }
...
class Uart:
    ...
    def __init__(
        ...
        assert controlFlow in self.controlFlowMapping, (
            f"Unsupported control flow: {controlFlow}"
        )
