            ...
            for chunk in self.chunks:
                with open(chunk.path, "rb") as fin:
                    ...
                    if chunk.isMaxSize:
                        ...
                        assert fin.tell() <= chunk.maxSize, (
                            f"The chunk '{chunk}' is too large ({fin.tell()}) to fit into the binary."
                        )
...
def makeBinary(
    chunks: typing.Iterable[str], size: typing.Optional[str] = None
) -> Binary:
    ...
    for chunk in chunks:
        parts = chunk.split(",")
        assert len(parts) >= 2, (
            f"--chunk needs at least the path and the address in the following format: 'path,address', not '{chunk}'."
        )
        assert len(parts) <= 3, (
            f"--chunk cannot have more than 3 entities: 'path,address,max-size', not '{chunk}'."
        )

        binary.chunks.append(
            Chunk(
                pathlib.Path(parts[0]),
                int(parts[1], 0),
    ...
    for nextChunkIndex in range(1, len(binary.chunks)):
        ...
        if chunkObject.isMaxSize:
            assert chunkObject.isMaxSize <= availableSize, (
                f"The maximum size defined for {chunkObject}, is larger the available size ({availableSize}) between chunks (next chunk starts at address {nextChunk.address})."
            )
        else:
    ...
    if len(binary.chunks) > 0 and binary.isSize:
        ...
        assert availableSize > 0, (
            f"There is no size available for the last chunk: {availableSize} given the size constraint of {binary.size}."
        )
        if chunkObject.isMaxSize:
            assert chunkObject.isMaxSize <= availableSize, (
                f"The maximum size defined for {chunkObject}, is larger the available size ({availableSize}) for the last chunk given the size constraint of {binary.size}."
            )
        else:
            chunkObject.maxSize = availableSize
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--size", default=None, help="The total size of the binary in bytes."
    )
    parser.add_argument(
        "--chunk",
        dest="chunks",
    ...
    parser.add_argument(
        "output", type=pathlib.Path, help="Path of the file to be generated."
    )
