...
def localCommand(cmds: List[str], inputs: bytes = b"", ignoreFailure: bool = False, env: Optional[Dict[str, str]] = None, timeoutS: float = 60.) -> _ExecuteResult:
...
def localBazelBinary(path: str, args: List[str] = [], env: Dict[str, str] = {}, **kargs: Any) -> _ExecuteResult:
    env["RUNFILES_DIR"] = os.environ["RUNFILES_DIR"] if "RUNFILES_DIR" in os.environ else os.path.dirname(
        os.getcwd())
