    ...
    def addStdout(self, data: bytes) -> None:
        if data != b"":
            ...
            if self.stdout is not False:
                write = (
                    ExecuteResultStreamWriter._printToStdout
                    if isinstance(self.stdout, bool)
    ...
    def addStderr(self, data: bytes) -> None:
        if data != b"":
            ...
            if self.stderr is not False:
                write = (
                    ExecuteResultStreamWriter._printToStderr
                    if isinstance(self.stderr, bool)
...
class ExecuteResult:
    ...
    def getStdout(self) -> str:
        return b"".join([entry[1] for entry in self.output if entry[0]]).decode(
            errors="ignore"
        )

    def getStderr(self) -> str:
        return b"".join([entry[1] for entry in self.output if not entry[0]]).decode(
            errors="ignore"
        )
...
class Cancellation:
    ...
    def __init__(self, timeBeforeKillS: float = 5.0) -> None:
...
def localCommand(
    ...
    try:
        ...
        while isRunning and timer.is_alive():
            for key, _ in sel.select():
                ...
                if not data:
                    isRunning = False
                (stream.addStdout if key.fileobj is proc.stdout else stream.addStderr)(
                    data
                )
    ...
    assert ignoreFailure or returnCode == 0, (
        f"While executing: {shlex.join(cmds)}\n{result}"
    )
...
def localPython(
    script: str, args: Optional[List[str]] = None, **kwargs: Any
) -> ExecuteResult:
...
def localBash(
    script: bytes, args: Optional[List[str]] = None, **kwargs: Any
) -> ExecuteResult:
