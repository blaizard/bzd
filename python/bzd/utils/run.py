...
class ExecuteResult:
	...
	def getStdout(self) -> str:
		return b"".join([entry[1] for entry in self.output if entry[0]]).decode(errors="ignore")

	def getStderr(self) -> str:
		return b"".join([entry[1] for entry in self.output if not entry[0]]).decode(errors="ignore")
...
def localCommand(
	...
	Args:
	                ...
	                ignoreFailure: If set to True, failures (return code != 0) are ignored. Otherwise, it will throw.
