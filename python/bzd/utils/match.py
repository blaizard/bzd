...
def getMatchingWeight(word: str, compareWith: str) -> float:
    ...
    while True:
        ...
        curWeight = 1.0 if start == 0 else 0.9
        iSentence = start
...
def sortMatchingWeight(
    word: str, compareWithList: typing.List[str], minWeight: float = 0.5
) -> typing.List[str]:
    weightList = [
        (
            compareWith,
    ...
    weightFilteredList = [
        (compareWith, weight)
        for compareWith, weight in weightList
        if weight >= minWeight
    ]
    weightSortedList = sorted(
        weightFilteredList, key=lambda item: item[1], reverse=True
    )
