...
class _SSHInteractiveHandle:
    ...
    def command(
        ...
        stream = ExecuteResultStreamWriter(
            stdout=stdout, stderr=stderr, maxSize=maxOutputSize
        )
...
class SSH:
    ...
    def upload(self, source: pathlib.Path, destination: pathlib.Path) -> None:
        ...
        command = (
            ...
            + [
                ...
                str(self.port),
                str(source),
                f"{self.username}@{self.host}:{destination}",
            ]
        )
    ...
    @contextmanager
    def forwardPort(
        self, port: int, waitHTTP: typing.Optional[str] = None, waitTimeoutS: int = 20
        ...
        try:
            ...
            while True:
                ...
                if timeLeftS <= 0:
                    raise TimeoutError(
                        f"Port forwarding timed-out within {waitTimeoutS}s."
                    )
                if waitHTTP:
                    try:
