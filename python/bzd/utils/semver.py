...
class _SemverConstraint:
    ...
    def _matchTilde(self, version: Semver) -> bool:
        minVersion = Semver(
            major=self.setIfWildcard(self.major, 0),
        ...
        maxVersion = Semver(
            major=self.setIfWildcard(self.major, _MAX_VERSION_INT),
    ...
    def _matchVersion(self, version: Semver) -> bool:
        if self.isWildcard(self.major):
            return version >= Semver(
                major=0, minor=0, patch=0, preRelease=self.preRelease, build=self.build
            )
    ...
    def match(self, version: Semver) -> bool:
        ...
        if self.operator:
            assert self.operator in matcherMap, (
                f"The operator '{self.operator}' is not supported."
            )
    ...
    def match(self, version: Semver) -> bool:
        ...
        while len(constraints) > 0:
            ...
            if isinstance(constraint, _SemverConstraint):
                ...
                assert len(stack) >= 1, (
                    f"The binary operator {constraint} is missing its first argument."
                )
                assert len(constraints) >= 1, (
                    f"The binary operator {constraint} is missing its second argument."
                )
                secondArgument = constraints.pop(0)
                assert isinstance(secondArgument, _SemverConstraint), (
                    f"Compositions can only be used with constraints, not {secondArgument}"
                )
                result = binaryOperators[constraint](
                    stack.pop(), secondArgument.match(version)
                )
                stack.append(result)
            else:
        ...
        assert len(stack) == 1, (
            f"There should be a single data in the stack: {stack}, constraint: '{self.string}'"
        )
        return stack[0]
