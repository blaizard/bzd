...
from typing import Optional, Iterable, Any, Tuple, TextIO, Callable
import ctypes
import signal
import types
...
class _Context:
    ...
    def __init__(self) -> None:
        ...
        self.output: multiprocessing.queues.Queue[
            Tuple[bool, Optional[Any], str, Any]
        ] = multiprocessing.Queue()
        self.data: multiprocessing.queues.Queue[_WorkloadContext] = (
            multiprocessing.Queue()
        )
...
class WorkerContextManager:
    ...
    def __exit__(self, exc_type: Any, exc_value: Any, exc_tb: Any) -> None:
        ...
        if self.throwOnException and self.isException:
            raise Exception("At least one of the worker failed.")
    ...
    def __init__(
        ...
        self.workerList = [
            multiprocessing.Process(
                target=Worker._taskWrapper, args=(task, self.context)
            )
        ...
        while context.count.value > 0 or context.stop.value == 0:
            ...
            if workloadContext.timeoutS > 0:
                ...
                def _timeoutHandler(
                    ...
                    raise TimeoutError(
                        f"Function timed out after {workloadContext.timeoutS}s."
                    )
            ...
            context.output.put(
                (isException, result, stdout.getvalue(), workloadContext.data)
            )
