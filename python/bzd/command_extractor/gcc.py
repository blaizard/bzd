...
class CommandExtractorGcc(CommandExtractor):
    ...
    def addLibrary(self, name: str, factory: ItemFactory = ItemFactory()) -> None:
        ...
        fileName = (
            name
            if re.match(r"lib.*\.a", pathlib.Path(name).name)
            else "lib{}.a".format(name)
        )
        for path in self.librarySearchPaths:
            fullPath = path / fileName
            if fullPath.is_file():
                self.result.append(
                    factory.make(ItemLibrary, Categories.library, fullPath)
                )
                return
    ...
    def addLinkerScript(
        self, fileName: str, factory: ItemFactory = ItemFactory()
    ) -> None:
        ...
        for path in self.librarySearchPaths:
            ...
            if fullPath.is_file():
                self.result.append(
                    factory.make(ItemPathOrString, Categories.linkerScript, fullPath)
                )
                return
        self.result.append(
            factory.make(ItemPathOrString, Categories.linkerScript, fileName)
        )
    ...
    def addOutputPath(
        self, path: pathlib.Path, factory: ItemFactory = ItemFactory()
    ) -> None:
        """Add the output path."""

        self.result.append(
            factory.make(ItemPath, Categories.outputPath, self.cwd / path)
        )
            ...
            [
                {
                    r"-L": Processor(
                        1,
                        lambda factory, x: self.addSearchPath(
                            Categories.librarySearchPath, pathlib.Path(x), factory
                        ),
                    ),
                    r"-I": Processor(
                        1,
                        lambda factory, x: self.addSearchPath(
                            Categories.includeSearchPath, pathlib.Path(x), factory
                        ),
                    ),
                },
                {
                    r"-l": Processor(1, lambda factory, x: self.addLibrary(x, factory)),
                    r"-T": Processor(
                        1, lambda factory, x: self.addLinkerScript(x, factory)
                    ),
                    r"-o": Processor(
                        1, lambda factory, x: self.addOutputPath(x, factory)
                    ),
