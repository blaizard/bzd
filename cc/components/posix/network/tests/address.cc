...
TEST(Address, fromIpV6)
{
	EXPECT_FALSE(Address::fromIpV6(protocol::udp, "0.0.0.0", 32));
	EXPECT_FALSE(Address::fromIpV6(protocol::udp, "not valid ip", 32));
	EXPECT_TRUE(Address::fromIpV6(protocol::udp, "::1", 32));
}
...
TEST(Address, fromIp)
{
	...
	EXPECT_TRUE(Address::fromIp(protocol::udp, "::1", 32));
}
