    ...
    def visitSymbol(
        ...
        useReference = (
            ...
            if self.isTopLevel
                ...
                symbol.category == Category.interface
        ...
        for index, fqn in enumerate(symbol.kinds):
            if fqn in builtins:
                if callable(builtins[fqn].toType):
                    output, nested = builtins[fqn].toType(
                        symbol, nested, useReference, self.values
                    )
                else:
        ...
        if self.isTopLevel:
            if self.definition:
                if (
                    symbol.underlyingTypeFQN in builtins
                    and builtins[symbol.underlyingTypeFQN].constexpr
                ):
