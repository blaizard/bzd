...
class TestRun(unittest.TestCase):
    ...
    def render(self, template: str) -> str:
        ...
        return Template(
            ...
            + template
        ).render(composition, Transform(composition=composition))

    def testSymbolToStr(self) -> None:
        output = self.render(
            """{{ entity("MyStruct.var").typeResolved | symbolToStr }}"""
        )
        ...
        output = self.render(
            """{{ entity("myComponent").typeResolved | symbolToStr }}"""
        )
        self.assertEqual(output, "MyComponent<bzd::Float32>")

    def testSymbolReferenceToStr(self) -> None:
        output = self.render(
            """{{ entity("MyStruct.var").typeResolved | symbolReferenceToStr }}"""
        )
        self.assertEqual(output, "bzd::Int32&")

        output = self.render(
            """{{ entity("myVar").typeResolved | symbolReferenceToStr }}"""
        )
        self.assertEqual(output, "const bzd::Int32&")

        output = self.render(
            """{{ entity("myStruct").typeResolved | symbolReferenceToStr }}"""
        )
        self.assertEqual(output, "MyStruct&")

        output = self.render(
            """{{ entity("myComponent").typeResolved | symbolReferenceToStr }}"""
        )
        self.assertEqual(output, "MyComponent<bzd::Float32>&")

    def testSymbolDefinitionToStr(self) -> None:
        output = self.render(
            """{{ entity("MyStruct.var").typeResolved | symbolDefinitionToStr }}"""
        )
        self.assertEqual(output, "constexpr bzd::Int32")

        output = self.render(
            """{{ entity("myVar").typeResolved | symbolDefinitionToStr }}"""
        )
        self.assertEqual(output, "const constexpr bzd::Int32")

        output = self.render(
            """{{ entity("myStruct").typeResolved | symbolDefinitionToStr }}"""
        )
        self.assertEqual(output, "MyStruct")

        output = self.render(
            """{{ entity("myComponent").typeResolved | symbolDefinitionToStr }}"""
        )
        self.assertEqual(output, "MyComponent<bzd::Float32>")

    def testSymbolNonConstToStr(self) -> None:
        output = self.render(
            """{{ entity("MyStruct.var").typeResolved | symbolNonConstToStr }}"""
        )
        self.assertEqual(output, "bzd::Int32")

        output = self.render(
            """{{ entity("myVar").typeResolved | symbolNonConstToStr }}"""
        )
        self.assertEqual(output, "bzd::Int32")

        output = self.render(
            """{{ entity("myStruct").typeResolved | symbolNonConstToStr }}"""
        )
        self.assertEqual(output, "MyStruct")

        output = self.render(
            """{{ entity("myComponent").typeResolved | symbolNonConstToStr }}"""
        )
        self.assertEqual(output, "MyComponent<bzd::Float32>")

    def testSymbolRegistryToStr(self) -> None:
        output = self.render(
            """{{ entity("MyStruct.var").typeResolved | symbolRegistryToStr }}"""
        )
        self.assertEqual(output, "bzd::Int32")

        output = self.render(
            """{{ entity("myVar").typeResolved | symbolRegistryToStr }}"""
        )
        self.assertEqual(output, "const bzd::Int32")

        output = self.render(
            """{{ entity("myStruct").typeResolved | symbolRegistryToStr }}"""
        )
        self.assertEqual(output, "MyStruct")

        output = self.render(
            """{{ entity("myComponent").typeResolved | symbolRegistryToStr }}"""
        )
        self.assertEqual(output, "MyComponent<bzd::Float32>")
