...
from bdl.entities.all import Namespace, Expression
...
from cc.bdl.generator.impl.value import (
    valueToStr as valueToStrOriginal,
    literalNativeToStr,
)
...
class Transform:
    ...
    def inheritanceToStr(self, inheritanceList: typing.List[Symbol]) -> str:
        return ", ".join(
            [
                f"public {str(symbolToStrOriginal(inheritance))}<Impl>"
    ...
    def inheritanceAdapterToStr(self, inheritanceList: typing.List[Symbol]) -> str:
        return ", ".join(
            [
                "public {}<Impl>".format(
                    str(symbolToStrOriginal(inheritance, adapter=True))
                )
                for inheritance in inheritanceList
            ]
        )
    ...
    def paramsDeclarationToList_(
        self, params: ParametersResolved, isRegistry: bool = False
    ) -> typing.List[str]:
        ...
        return [
            valueToStrOriginal(item, symbols=symbols, registry=isRegistry)
            for item in params
        ]
    ...
    def paramsDesignatedInitializers(self, params: ParametersResolved) -> str:
        ...
        values = {
            item.name: valueToStrOriginal(item, includeComment=False) for item in params
        }
    ...
    def paramsDeclaration(self, params: ParametersResolved, isRegistry: bool) -> str:
        paramsToList = self.paramsDeclarationToList_(
            params=params, isRegistry=isRegistry
        )
        if len(paramsToList) < 2:
            return ", ".join(paramsToList)
        return ",\n".join(paramsToList)

    def paramsFilterOutLiterals(
        self, params: ParametersResolved
    ) -> typing.Iterator[ParametersResolvedItem]:
        for item in params:
            if item.param.isLiteral:
                continue
            yield item
    ...
    def expressionToDefinition(self, entity: Expression) -> str:
        ...
        output = ""
        values = (
            self.paramsDeclarationToList_(params=entity.parametersResolved)
            if entity.parametersResolved.size()
            else []
        )
        entity.assertTrue(
        ...
        for name, metadata in connections.items():
            ...
            # Use stubs if no connection exists.
                ...
                if kind == "sink":
                    ...
                    assert len(metadata["connections"]), (
                        f"Must be only one connection, {metadata['connections']}"
                    )
                else:
...
def formatCc(bdl: Object, data: typing.Optional[Path] = None) -> str:
    template = Template.fromPath(
        Path(__file__).parent / "template/file.h.btl", indent=True
    )
...
def compositionCc(
    ...
    template = Template.fromPath(
        Path(__file__).parent / "template/composition.cc.btl", indent=True
    )

    for target, composition in compositions.items():
        content = template.render(
            composition, Transform(composition=composition, data=data)
        )
