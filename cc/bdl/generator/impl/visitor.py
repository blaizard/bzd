...
class Transform:
	...
	def inheritanceAdapterToStr(self, inheritanceList: typing.List[Symbol]) -> str:
		return ", ".join(
			["public {}<Impl>".format(str(symbolToStrOriginal(inheritance, adapter=True))) for inheritance in inheritanceList]
		)
	...
	def expressionToDefinition(self, entity: Expression) -> str:
		...
		"""
		output = ""
		values = self.paramsDeclarationToList_(params=entity.parametersResolved) if entity.parametersResolved.size() else []
		entity.assertTrue(
