...
TEST(FloatingPoint, Float32)
{
	...
	{
		bzd::serialize(string.assignerScope(), bzd::Float32{3.14});
...

	{
		...
		bzd::serialize(shortString.assignerScope(), bzd::Float32{3.14});
...
TEST(FloatingPoint, Float64)
{
	...
	{
		bzd::serialize(string.assignerScope(), bzd::Float64{3.14});
	...
	{
		bzd::String<1u> shortString;
		bzd::serialize(shortString.assignerScope(), bzd::Float64{3.14});
		EXPECT_EQ(shortString.size(), 1u);
