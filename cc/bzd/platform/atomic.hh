...
public:
	...
	constexpr void store(const T& v) noexcept { this->get().store(v); }

	constexpr T load() const noexcept { return this->get().load(); }
	...
	constexpr T exchange(T v) noexcept { return this->get().exchange(v); }

	constexpr bool compareExchange(T& expected, T desired) noexcept { return this->get().compare_exchange_strong(expected, desired); }
};
} // namespace bzd
