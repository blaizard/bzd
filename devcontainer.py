...
class DevelopmentContainer:
    def __init__(
        self, features: typing.Sequence[Feature], temporaryPath: pathlib.Path
    ) -> None:
    ...
    def dockerComposePath(self) -> pathlib.Path:
        return (
            self.temporaryPath
            / f"bzd-devcontainer-{self.namePrefix}"
            / "docker-compose.yaml"
        )
    ...
    def dockerFile(self) -> str:
        ...
        if self.userNamespaceRemapping:
            ...
            instructions += [
                f"RUN getent group {gid} || groupadd -g {gid} '{name}'"
        ...
        instructions += [
            instruction
            for feature in self.features
            for instruction in feature.dockerFile
        ]
        instructionsStr = "\n".join(instructions)
...
"""
    ...
    def dockerCompose(self) -> str:
        ...
        volumes += [
            f"{file}:{file}"
            for file in [self.home / ".bash_history", self.home / ".netrc"]
            if file.is_file()
        ]
        ...
        extraService += [
            line
            for feature in self.features
            for line in feature.dockerCompose.get("service", [])
        ]
        ...
        extraVolumes = "\n".join(
            [
                line
                for feature in self.features
                for line in feature.dockerCompose.get("volumes", [])
            ]
        )
        if extraVolumes:
            extraStr += "volumes:\n" + "\n".join(
                [f"  {line}" for line in extraVolumes.splitlines()]
            )
...
"""
    ...
    def run(self, args: typing.List[str], env: typing.List[str]) -> None:
        ...
        workdir = (
            self.cwd if self.cwd.is_relative_to(self.workspace) else self.workspace
        )
        subprocess.run(
            [
                "docker",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--build", action="store_true", help="Re-build the container if set."
    )
    parser.add_argument(
        "--build-force",
        action="store_true",
