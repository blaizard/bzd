...
class ContainerState:
    ...
    def printLogs(self, handle: typing.Any) -> None:
        ...
        handle.command(
            ["docker", "logs", "--tail", "50", self.getId()], stdout=True, stderr=True
        )
...
def getStatesFromDockerCompose(
    handle: typing.Any, dockerComposeFile: pathlib.Path
) -> typing.Dict[str, ContainerState]:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--registry-port", type=int, default=5000, help="Port for the registry."
    )
    parser.add_argument(
        "--version",
        type=str,
    ...
    parser.add_argument(
        "transport", type=str, help="Location where to deploy the target."
    )
    ...
    registry = Template.fromPath(
        pathlib.Path(__file__).parent / "registry.yml.btl"
    ).render(common)
    ...
    with transport.session() as handle:
        ...
        with usingDockerRegistry(
            ...
            result = handle.command(
                ["find", str(applicationsDirectory), "-maxdepth", "1", "-type", "d"]
            )
            ...
            for application, content in dockerCompose.items():
                ...
                handle.uploadContent(
                    content, f"{dockerComposeDirectory / args.version}.yml"
                )
                ...
                dockerComposeFiles = sorted(
                    [
                        s
                        for s in handle.command(
                            ["ls", "-1", str(dockerComposeDirectory)]
                        )
                        .getOutput()
                        .split("\n")
                        if s.endswith(".yml")
                ...
                allDockerComposeFiles += [
                    dockerComposeDirectory / name for name in keepDockerComposeFiles
                ]
            ...
            with handle.forwardPort(
                ...
                usedImages = getAllImagesFromDockerComposeFiles(
                    handle, allDockerComposeFiles
                )
                dockerRegistry = DockerRegistry(
                    f"http://localhost:{args.registry_port}"
                )
