...
class DockerCompose:
    ...
    def _getService(self, service: str) -> typing.Dict[str, typing.Any]:
        ...
        assert service in self.data["services"], (
            f"The service '{service}' is not present in the docker-compose.yaml file."
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="Display the output as it goes."
    )
    parser.add_argument("service", type=str, help="The name of the service.")
    parser.add_argument(
        "docker_compose",
        type=pathlib.Path,
        help="The docker-compose file in json format.",
    )
    ...
    if exposedPort is not None:
        scheduler.add(
            name="ping", intervalS=0.5, callback=httpServerReady, args=(exposedPort,)
        )
    scheduler.start()
    ...
    with tempfile.NamedTemporaryFile(delete_on_close=False) as f:
        ...
        try:
            ...
            localCommand(
                ["docker", "compose", "--file", f.name, "down", "--remove-orphans"],
                ignoreFailure=True,
            )
