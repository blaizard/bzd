...
class Converters:
    ...
    def allExtensions(self) -> typing.Set[str]:
        return {
            ...
            for value in converter.extensions  # type: ignore
        }
    ...
    def _getOutput(
        self, path: pathlib.Path, outputFormat: OutputFormat
    ...
    def _run(self, commands: typing.Sequence[str]) -> typing.Optional[str]:
        result = subprocess.run(
            commands, text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT
        )
    ...
    def _moveFiles(
        self, original: pathlib.Path, file: pathlib.Path, output: pathlib.Path
    ) -> None:
    ...
    def _processFile(
        ...
        for converter in self.converters:
            if extension not in converter.extensions:  # type: ignore
                continue
            print(
                f"Converting {path} ({self._sizeToString(path)}) with {converter.__class__.__name__}..."
            )
            ...
            try:
                ...
                if result in {ConversionResult.completed, ConversionResult.skipped}:
                    assert outputTemporary.is_file(), (
                        f"The output '{outputTemporary}' was not created."
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--format", default="jpeg", type=OutputFormat, help="The format type."
    )
