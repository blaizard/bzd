...
class ActionInterface(typing.Protocol):
    ...
    def __init__(self) -> None:
        assert self.ProviderInput is not None, "The ProviderInput must be set."
        assert self.ProviderOutput is not None, "The ProviderOutput must be set."

    def process(
        self, input: ProviderInput, directory: pathlib.Path, context: FlowContext
    ) -> typing.List[typing.Tuple[ProviderOutput, typing.Optional[FlowEnum]]]:
...
class FlowState:
    ...
    def toJson(self) -> typing.Dict[str, typing.Any]:
        ...
        return {
            "flow": self.flow,
            "provider": providerSerialize(self.provider),
            "index": self.index,
        }
    ...
    def fromJson(data: typing.Dict[str, typing.Any]) -> "FlowState":
        return FlowState(
            provider=providerDeserialize(data["provider"]),
            flow=data["flow"],
            index=data["index"],
        )
...
class FlowRegistry:
    def __init__(
        self, schema: typing.Type[FlowEnum], actions: typing.Dict[str, ActionInterface]
    ) -> None:
        self.schema = schema
        ...
        for flow in self.schema:
            for actionName in flow.value:
                assert actionName in self.actions, (
                    f"The action named '{actionName}' does not exists."
                )

    def restoreOrReset(
        self, directory: pathlib.Path, flow: FlowSchemaType, provider: Provider
    ) -> "Flow":
        ...
        if Flow.stateJson(directory).exists():
            try:
                ...
                print(
                    f"Could not restore previous flow: {str(e)}, resetting flow state."
                )
...
class Flow:
    ...
    def run(self, uid: str) -> typing.List[Provider]:
        ...
        while any(not state.completed() for state in self.states):
            ...
            workingDirectory = (
                self.directory
                / f"{(documentIndex + 1):02}.{(state.index + 1):02}.{actionName}"
            )
