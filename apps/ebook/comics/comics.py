...
class Comics(ActionInterface):
    ...
    def outliers(
        self, images: typing.List[pathlib.Path], coefficient: float
        ...
        nGramsCounterNormalized = {
            item: count / len(images) for item, count in nGramsCounter.items()
        }
        ...
        for image, nGrams in nGramsAll.items():
            scores[image] = sum(
                [nGramsCounterNormalized[gram] for gram in nGrams]
            ) / len(nGrams)
        ...
        if len(scores) >= 2:
            ...
            while True:
                ...
                scoresOutliers = {
                    image: value for image, value in scores.items() if value < threshold
                }
        ...
        return outliers, [image for image in images if image not in outliers]

    def processDirectory(
        self, provider: ProviderEbook, directory: pathlib.Path
        ...
        for subdir, dirs, files in os.walk(directory):
            for file in files:
                ...
                try:
                    ...
                    continue
                allImagesByDirectory.setdefault(subdirectory, []).append(
                    subdirectory / file
                )

        outputs: typing.List[
            typing.Tuple[ProviderImages, typing.Optional[FlowEnum]]
        ] = []
        for subdirectory, allImages in allImagesByDirectory.items():
            ...
            if len(allImages):
                outputs.append(
                    (ProviderImages(images=allImages, metadata=provider.metadata), None)
                )
