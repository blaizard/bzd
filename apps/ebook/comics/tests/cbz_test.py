...
class TestRun(unittest.TestCase):
    def assertProviderEbook(
        self, provider: ProviderImages, expectedImages: typing.List[str]
    ) -> None:
        self.assertEqual(
            len(provider.images),
            len(expectedImages),
            "There is not the same number of images.",
        )
        for image, expected in zip(provider.images, expectedImages):
            self.assertTrue(
                str(image).endswith(expected),
                f"Expected '{expected}' but got '{image}'.",
            )
    ...
    def testSample1(self) -> None:
        ...
        provider = ProviderEbook(
            ebook=pathlib.Path(__file__).parent / "data/sample1.cbz"
        )
        with tempfile.TemporaryDirectory() as directory:
            result = cbz.process(
                provider=provider,
                directory=pathlib.Path(directory),
                context=FlowContext.forTesting(),
            )
            self.assertEqual(len(result), 1)
            self.assertProviderEbook(
                result[0][0],
                expectedImages=[
                    "file001.jpg",
                    "file002.jpg",
    ...
    def testSample2(self) -> None:
        ...
        cbz = Cbz()
        provider = ProviderEbook(
            ebook=pathlib.Path(__file__).parent / "data/sample2.cbz"
        )
        with tempfile.TemporaryDirectory() as directory:
            result = cbz.process(
                provider=provider,
    ...
    def testSample3(self) -> None:
        ...
        cbz = Cbz()
        provider = ProviderEbook(
            ebook=pathlib.Path(__file__).parent / "data/sample3.cbz"
        )
        with tempfile.TemporaryDirectory() as directory:
            result = cbz.process(
                provider=provider,
    ...
    def testSample4(self) -> None:
        ...
        cbz = Cbz()
        provider = ProviderEbook(
            ebook=pathlib.Path(__file__).parent / "data/sample4.cbz"
        )
        with tempfile.TemporaryDirectory() as directory:
            result = cbz.process(
                provider=provider,
    ...
    def testSample5(self) -> None:
        ...
        cbz = Cbz()
        provider = ProviderEbook(
            ebook=pathlib.Path(__file__).parent / "data/sample5.cbz"
        )
        with tempfile.TemporaryDirectory() as directory:
            result = cbz.process(
                provider=provider,
    ...
    def testSample6(self) -> None:
        ...
        cbz = Cbz()
        provider = ProviderEbook(
            ebook=pathlib.Path(__file__).parent / "data/sample6.cbz"
        )
        with tempfile.TemporaryDirectory() as directory:
            result = cbz.process(
                provider=provider,
    ...
    def testSample7(self) -> None:
        ...
        cbz = Cbz()
        provider = ProviderEbook(
            ebook=pathlib.Path(__file__).parent / "data/sample7.cbz"
        )
        with tempfile.TemporaryDirectory() as directory:
            result = cbz.process(
                provider=provider,
