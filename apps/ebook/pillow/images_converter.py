...
class ImagesConverter(ActionInterface):
    ...
    def __init__(
        self, maxDPI: typing.Optional[int] = None, scale: typing.Optional[float] = None
    ) -> None:
        super().__init__()
        self.maxDPI = maxDPI
        self.scale = scale
        assert self.maxDPI is None or self.scale is None, (
            "Cannot specify both maxDPI and scale, choose one."
        )

    def process(
        self, provider: ProviderImages, directory: pathlib.Path, context: FlowContext
        ...
        for index, path in enumerate(provider.images):
            with Image.open(path) as image:
                ...
                if self.maxDPI is not None:
                    ...
                    if maybeDPI is not None:
                        comments.append(
                            f"image DPI {int(maybeDPI[0])}x{int(maybeDPI[1])}"
                        )
                        if maybeDPI[0] > self.maxDPI or maybeDPI[1] > self.maxDPI:
                            ...
                            assert scaleDPI <= scale, (
                                f"Scaling by DPI must be less than or equal to {scale}, got {int(scaleDPI)}."
                            )
                ...
                assert scale <= 1.0, (
                    f"Scale must be less than or equal to 1.0, got {scale}."
                )
                if scale < 1.0:
