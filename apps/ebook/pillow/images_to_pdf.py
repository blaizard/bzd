...
class ImagesToPdf(ActionInterface):
    ...
    def process(
    ...
    ) -> typing.List[typing.Tuple[ProviderPdf, typing.Optional[FlowEnum]]]:
        ...
        if provider.metadata.date:
            metadata["creationDate"] = provider.metadata.date.strftime(
                "D:%Y%m%d%H%M%S%z"
            )

        assert len(images) > 0, "There must be at least one image from the ebook."
