...
class EPub(ActionInterface):
    ...
    def __init__(
        self, driver: typing.Any = DriverSelenium(), zoom: float = 1.0
    ) -> None:
        super().__init__()
        self.driver = driver
        self.zoom = zoom
    ...
    def parseDate(string: str) -> typing.Optional[datetime.datetime]:
        ...
        if maybeMatch:
            ...
            return datetime.datetime(
                int(d["year"]), int(d["month"] or 1), int(d["day"] or 1)
            )
        return None
    ...
    def readMetadata(self, dirname: pathlib.Path) -> typing.List[EPubMetadata]:
        ...
        root = tree.getroot()
        rootfiles = [
            elt.attrib["full-path"]
            for elt in root.iter()
            if elt.tag.endswith("rootfile")
        ]
        ...
        for rootfile in rootfiles:
            ...
            manifest = {
                ...
                for manifest in [
                    elt for elt in root.iter() if elt.tag.endswith("manifest")
                ]
            ...
            for spine in [elt for elt in root.iter() if elt.tag.endswith("spine")]:
                ...
                documentMetadata.document = [
                    manifest[item.attrib["idref"]] for item in spine
                ]
    ...
    def process(
        ...
        with self.driver as driver:
            ...
            for documentNumber, metadata in enumerate(documents):
                ...
                outputs.append(
                    (ProviderImages(images=output, metadata=metadata.metadata), None)
                )
