...
from apps.ebook.flow import (
    ActionInterface,
    FlowRegistry,
    FlowEnum,
    FlowContext,
)
from apps.ebook.providers import (
    ProviderEbook,
    ProviderEbookMetadata,
)
...
class Discover(ActionInterface):
    ...
    def process(
    ...
    ) -> typing.List[typing.Tuple[ProviderEbook, typing.Optional[FlowEnum]]]:

        ebookFormat = (
            self.ebookFormat or provider.ebook.suffix.removeprefix(".").lower()
        )
        ...
        assert False, (
            f"Unsupported ebook extension '.{ebookFormat}', only the following are supported for this flow: {str(', '.join([value.name for value in context.schema]))}."
        )
...
def runFlow(
    provider: ProviderEbook, flowRegistry: FlowRegistry, directory: pathlib.Path
) -> None:
    """Run the given flow."""
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Assume a specific format for the ebook.",
    )
    parser.add_argument(
        "--coefficient",
        type=float,
    ...
    parser.add_argument(
        "--zoom", type=float, default=1.0, help="Zoom coefficient to be used."
    )
    parser.add_argument(
        "--clean", action="store_true", help="Start from a clean sandbox if any."
    )
    parser.add_argument(
        "--max-dpi", type=int, default=None, help="Maximum DPI for an image."
    )
    parser.add_argument(
        "--scale",
        type=float,
    ...
    parser.add_argument(
        "ebook", type=pathlib.Path, help="The ebook file or directory to sanitize."
    )
    ...
    if args.ebook.is_file():
        ...
        ebooks = [
            f for f in allFiles if hasattr(schema, f.suffix.removeprefix(".").lower())
        ]
    ...
    for index, ebook in enumerate(ebooks):
        directory = (
            ...
            else SandboxDirectory(path=args.sandbox)
        )
        try:
            print(
                f"=== [{index + 1}/{len(ebooks)}] Running flow '{args.action}' on {ebook} ==="
            )
