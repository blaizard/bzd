...
class Context:
    ...
    def parse(
        ...
        parser.add_argument(
            "--bootloader-update-interval",
            type=float,
            help="Application update interval in seconds.",
        )
        parser.add_argument(
            "--bootloader-update-policy",
            type=StablePolicy,
        ...
        parser.add_argument(
            ...
            type=pathlib.Path,
            help="Path to get the update form remote.",
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "--bootloader-token",
            type=str,
            help="The node application token to be used.",
        )
        parser.add_argument(
            "--bootloader-uid", type=str, help="The unique identifier of this instance."
        )
        parser.add_argument(
            "--bootloader-application",
            type=pathlib.Path,
            help="The application to be called by the bootloader.",
            default=pathlib.Path(application)
            if application
            else (pathlib.Path(__file__).parent / "tests/noop"),
        )
    ...
    def updatePath(self) -> typing.Optional[pathlib.Path]:
        return typing.cast(
            typing.Optional[pathlib.Path], self.values.bootloader_update_path
        ) or pathlib.Path(updatePath)

    @property
    def updateIgnore(self) -> typing.Optional[str]:
        return self.updateIgnoreOverride or typing.cast(
            typing.Optional[str], self.values.bootloader_update_ignore
        )

    @property
    def stablePath(self) -> typing.Optional[pathlib.Path]:
        return typing.cast(
            typing.Optional[pathlib.Path], self.values.bootloader_stable_path
        )

    @property
    def selfPath(self) -> typing.Optional[pathlib.Path]:
        return typing.cast(
            typing.Optional[pathlib.Path], self.values.bootloader_self_path
        )
    ...
    def argsForBinary(self, binary: pathlib.Path) -> typing.List[str]:
        ...
        if self.values.bootloader_update_interval:
            args += [
                "--bootloader-update-interval",
                str(self.values.bootloader_update_interval),
            ]
        if self.values.bootloader_update_policy:
            args += [
                "--bootloader-update-policy",
                str(self.values.bootloader_update_policy),
            ]
        if self.values.bootloader_update_policy:
            args += [
                "--bootloader-update-path",
                str(self.values.bootloader_update_path),
            ]
        if self.values.bootloader_stable_path:
            args += [
                "--bootloader-stable-path",
                str(self.values.bootloader_stable_path),
            ]
        ...
        if self.values.bootloader_application:
            args += [
                "--bootloader-application",
                str(self.values.bootloader_application),
            ]
        args += self.rest
        return args
    ...
    def switchProcessToBinary(self, path: pathlib.Path, args: typing.List[str]) -> None:
        ...
        assert os.access(path, os.X_OK), (
            f"File '{path}' does not have execution permissions."
        )
...
class ContextCustomRelease(Context):
    def __init__(
        self, args: typing.List[str], logger: Logger, release: typing.Any = None
    ) -> None:
...
def autoUpdateApplication(context: Context, path: pathlib.Path, uid: str) -> None:
    ...
    maybeUpdate = context.release.fetch(
        path=str(path), uid=uid, ignore=context.updateIgnore
    )
    if maybeUpdate:
        # Create a directory with the uid to fetch the last update.
        updatePath = RollingNamedTemporaryFile(
            namespace=f"bootloader_{uid}", maxFiles=3
        ).get()
        ...
        binary = Binary(
            binary=updatePath, logger=Logger("binary").handlers(LoggerHandlerStub())
        )
        try:
            binary.run()
    ...
    def stableCallback() -> None:
        ...
        if context.stablePath:
            ...
            if context.stablePath != context.selfPath:
                context.logger.info(
                    f"Updating stable binary: {context.selfPath} -> {context.stablePath}"
                )
...
def runSelfTests(logger: Logger) -> None:
    ...
    release = {
        "noop": [{"name": "first", "binary": b"#!/bin/bash\nexit 0"}],
        "error": [{"name": "first", "binary": b"#!/bin/bash\nexit 1"}],
    }

    # Test happy path.
    context = ContextForTest(
        args=["--bootloader-application", noopBinary, "hello"], logger=logger
    )
    ...
    context = ContextForTest(
        release=release,
        args=["--bootloader-application", sleepBinary, "3600"],
        logger=logger,
    )
    ...
    context = ContextForTest(
        release=release,
        args=["--bootloader-application", noopBinary, "hello"],
        logger=logger,
    )
...
if __name__ == "__main__":
    ...
    if context.uid:
        with logger.handlersScope(
            LoggerHandlerAsyncBuffered(), context.node.makeLoggerHandler()
        ):
