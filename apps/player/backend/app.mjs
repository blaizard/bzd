...
function pathToPathList(path) { return path.split("/").filter(Boolean); }
...
(async () => {
  ...
  wss.on("connection", (ws) => {
	...
	terminal.on("data", (data) => { ws.send(data); });
  ...
  api.handle("get", "/scenario", async () => { return scenario.data; });

  api.handle("get", "/file/content", async (inputs) => { return await storage.read(pathToPathList(inputs.path)); });

  api.handle("post", "/file/content",
			 async (inputs) => { await storage.writeFromChunk(pathToPathList(inputs.path), inputs.content || ""); });

  api.handle("post", "/file/executable", async (inputs) => {
	await storage.setPermission(pathToPathList(inputs.path), new Permissions({
								  read : true,
								  write : true,
								  executable : true,
								}));
  ...
  api.handle("post", "/exec", async (inputs) => {
	...
	process.stdout.on("data", (data) => { stream.push(data); });
	process.stderr.on("data", (data) => { stream.push(data); });
	process.on("close", () => { stream.push(null); });
	return stream;
  });
