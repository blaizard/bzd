...
export default class Terminal {
  ...
  async init(config) {
	...
	this.process = spawn("apps/player/backend/terminal",
						 [
						   "--cwd",
						   this.cwd,
						   "--env",
	...
	// });
	//
	this.process.stdout.on("data", (data) => { this.event.trigger("data", data.toString()); });
	this.process.stderr.on("data", (data) => { this.event.trigger("data", data.toString()); });
	this.process.on("close", () => { this.event.trigger("exit"); });
