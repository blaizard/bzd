...
class Node {
  ...
  get expanded() { return this.node.expanded || false; }
  ...
  async refresh() { await this.filesystem.list(this.node.path); }
  ...
  setCursor(position) { this.node.cursor = position; }

  /// Checks if the node is a directory
  isFolder() { return FileSystem.isFolder(this.node); }

  /// Checks if the node is a file
  isFile() { return FileSystem.isFile(this.node); }
}
...
export default class FileSystem {
  ...
  get node() { return new Node(this, this.tree); }
  ...
  static basename(path) { return FileSystem.toPathList(path).at(-1); }
  ...
  static toPathList(path) { return path.trim().split("/").filter((name) => name.length > 0); }
  ...
  async list(path) {
	...
	Vue.set(node, "children", list.reduce((obj, item) => {
  ...
  /// Checks if the node is a directory
  static isFolder(node) { return node && "children" in node; }

  /// Checks if the node is a file
  static isFile(node) { return node && !("children" in node); }

  /// Refresh the tree.
  refresh() { this.tree.children = null; }

  makeNode_(node) { return new Node(this, node); }
}
