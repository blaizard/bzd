...
def _iterateByGroup(
    ...
    typing.Tuple[str, typing.Generator[OHLC, None, None]], None, None
]:
    ...
    while context["continue"]:
        ...
        def groupIterator(
            current: str, context: typing.Any
        ) -> typing.Generator[OHLC, None, None]:
            while True:
                yield context["ohlc"]
                try:
...
def recordingToPath(recording: Recording, path: pathlib.Path) -> None:
    ...
    for pair in recording:
        ...
        for key, iterator in _iterateByGroup(
            pair.__iter__(), group=lambda x: x.date.strftime("%Y-%m")
        ):
            ...
            with filePath.open(mode="w") as f:
                for ohlc in prices:
                    ...
                    content += "".join(
                        [f";{e.kind.value};{e.value}" for e in ohlc.events]
                    )
                    f.write(content + "\n")
