...
from apps.trader_python.recording.recording_from_data import (
    RecordingFromData,
    RecordingPairFromData,
)
...
class Generator:
    ...
    def generate(
        ...
        endTime: typing.Optional[Date] = None,
    ) -> RecordingFromData:
        ...
        start = (
            ...
            else Generator.dateToDatetime(startTime)
        )
        ...
        while start < end:
            endHistory = (
                ...
                if (start + datetime.timedelta(days=slicesDays)) <= end
                else end
            )
            ...
            for index in history.index:
                ...
                if history.loc[index, "Dividends"]:
                    events.append(
                        Event(EventKind.DIVIDEND, history.loc[index, "Dividends"])
                    )
                if history.loc[index, "Stock Splits"]:
                    events.append(
                        Event(EventKind.SPLIT, history.loc[index, "Stock Splits"])
                    )
