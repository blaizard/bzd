...
from apps.trader_python.recording.recording import (
    Recording,
    RecordingPair,
    Event,
    OHLC,
...
class RecordingPairFromData(RecordingPair):
    ...
    @staticmethod
    def fromPrices(
        prices: typing.Iterable[OHLC], *args: typing.Any, **kwargs: typing.Any
    ...
    def addOHLCObject(self, ohlc: OHLC, ignoreEqual: bool = False) -> None:
        ...
        index = bisect.bisect_left(
            self.prices, ohlc.timestamp, key=lambda x: x.timestamp
        )
...
class RecordingFromData(Recording):
    ...
    def addPair(self, recordingPair: RecordingPair) -> None:
        assert recordingPair.uid not in self.pairs, (
            f"The recording pair '{recordingPair.uid}' is already set."
        )
