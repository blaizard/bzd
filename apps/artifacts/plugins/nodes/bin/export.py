...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--children", type=int, default=99, help="Number of children to be used."
    )
    parser.add_argument(
        "--format", type=str, default="csv", choices=["csv"], help="Output format."
    )
    ...
    parser.add_argument(
        "--rename",
        type=str,
        help="Rename the output using the format-like schema. 'key' corresponds to the key as a list, while 'stem' to the expected file stem as a string.",
    )
    parser.add_argument(
        "--level",
        type=int,
        default=0,
        help="At which level from the given path the data should be extracted, by default everything will be a single file. If > 0, multiple files are created.",
    )
    ...
    if args.level > 0:
        ...
        for info in result["data"]:
            ...
            stem = "-".join(key)
            stem = (
                stem if args.rename is None else args.rename.format(key=key, stem=stem)
            )
    ...
    if after is not None:
        logger.info(
            f"Starting at {str(datetime.datetime.fromtimestamp(after / 1000.0, tz=datetime.timezone.utc))}"
        )
    if before is not None:
        logger.info(
            f"Ending at {str(datetime.datetime.fromtimestamp(before / 1000.0, tz=datetime.timezone.utc))}"
        )
