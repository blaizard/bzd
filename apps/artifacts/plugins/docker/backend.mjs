...
export default {
  ...
  services: {
	proxy: {
	  async is(params) { return params["docker.proxy"] == true;},
	  async start(params, context) {
		const proxy = DockerV2Proxy.makeFromStorageDockerV2(params["docker.proxy.url"], params["docker.proxy.port"],
															await context.getVolume());
		await proxy.start();
		return proxy;
	  },
	  async stop(proxy) { await proxy.stop();},
	},
  },
};
