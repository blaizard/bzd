	...
	def fetch(
		...
		for remote, retry, nbRetries in self.remotes:
			...
			try:
				...
				if retry == nbRetries:
					logger.warning(f"Exception while fetching {fullUrl}?{queryToString()} after {retry} retry: {str(e)}")
				continue
			...
			if update.name is None:
				logger.error(f"Every update must have a name, the one from {fullUrl}?{queryToString()} doesn't, ignoring.")
				continue
