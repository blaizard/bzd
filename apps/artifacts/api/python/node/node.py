...
class PublisherProtocol(typing.Protocol):
    def __call__(
        self, timestampMs: float, data: typing.Dict[str, typing.Any]
    ) -> None: ...
...
class Node(ArtifactsBase):
                ...
                try:
                    while len(self.buffer):
                        ...
                        self.httpClient.post(
                            url, json=content, query={"bulk": 1}, headers=headers
                        )
                        self.buffer.pop(0)
                    return
                    ...
                    if retry == nbRetries:
                        self.logger.warning(
                            f"Exception while publishing {url} after {nbRetries} retry: {str(e)}"
                        )
                    pass
    ...
    def _makeURI(
        ...
        assert actualUid is not None, "No UID were specified."
        assert endpoint in endpointToComponent, (
            f"The endpoint type '{endpoint}' does not exist."
        )
        subPath = "/".join([encodeURIComponent(s) for s in (self.path + (path or []))])
        return (
            f"/x/{volume or self.volume}/{endpointToComponent[endpoint]}{actualUid}/data"
            + (f"/{subPath}" if subPath else "")
            + "/"
        )

    def publishMultiNodes(
        self, data: typing.Dict[str, typing.Any], volume: typing.Optional[str] = None
    ) -> None:
    ...
    def publishBulk(
        ...
        volume: typing.Optional[str] = None,
        path: typing.Optional[typing.List[str]] = None,
        isClientTimestamp: bool = True,
    ) -> typing.Generator[PublisherProtocol, None, None]:
        ...
        uri = self._makeURI(uid=uid, volume=volume, path=path)
        self._publish(
            BufferEntryBulk(uri=uri, data=bulk, isClientTimestamp=isClientTimestamp)
        )
