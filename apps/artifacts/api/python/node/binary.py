...
class Utility:
    ...
    def cleanPathToList(self, data: str) -> typing.List[str]:
        ...
        for segment in data.split("/"):
            ...
            if segment == "..":
                self.assertTrue(
                    len(segments) > 0, f"The path '{data}' is going above the root."
                )
                segments.pop()
            else:
    ...
    def publish(self, data: str) -> None:
        ...
        self.assertTrue(
            len(pathData) > 0,
            f"There is no data to publish, the data given '{data}' evaluates to '.'",
        )
    ...
    def timeit(self, path: str, args: typing.List[str]) -> None:
        ...
        result = subprocess.run(
            args, cwd=os.environ.get("BUILD_WORKSPACE_DIRECTORY", None)
        )
        self.assertTrue(
            result.returncode == 0,
...
if __name__ == "__main__":
    ...
    parserPublish = subparsers.add_parser(
        "publish", help="Publish a data point to a node remote."
    )
    parserPublish.add_argument(
        "data", help="The path + data to publish separated by slashes."
    )

    parserPublishJson = subparsers.add_parser(
        "publish-json", help="Publish a data point to a node remote from a json file."
    )
    parserPublishJson.add_argument(
        "--path", help="Path of the data to access.", default="."
    )
    parserPublishJson.add_argument(
        "file", type=pathlib.Path, help="The path of the json file."
    )

    parserTimeit = subparsers.add_parser(
        "timeit", help="Time the given command and publish the result to the remote."
    )
