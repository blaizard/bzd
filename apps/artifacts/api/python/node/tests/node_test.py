...
class TestRun(unittest.TestCase):
    def _assertJsonBodyEqual(
        ...
        try:
            ...
            self.fail(
                f"Decoding valid JSON raised an unexpected error: {e}, with content: {body.decode()}"
            )
    ...
    def testPublish(self) -> None:
        ...
        def callback(
            method: str, url: str, body: typing.Optional[bytes], **kwargs: typing.Any
        ) -> None:
    ...
    def testPublishBulk(self) -> None:
        ...
        def callback(
            ...
            def check(data: typing.Any) -> None:
                self.assertEqual(
                    data["data"], [[1, {"hello": "world1"}], [2, {"hello": "world2"}]]
                )
                self.assertGreaterEqual(data["timestamp"], timestampStart)
                self.assertLessEqual(data["timestamp"], time.time() * 1000)
    ...
    def testPublishBulkFixeTimestamp(self) -> None:
        ...
        def callback(
            ...
            self.assertEqual(method, "POST")
            self.assertEqual(url, "http://localhost:8081/x/nodes/testuid/data/?bulk=1")
            self._assertJsonBodyEqual(
                body, {"data": [[1, {"hello": "world1"}], [2, {"hello": "world2"}]]}
            )
            self.calledCounter += 1
    ...
    def testPublishNoServerWithBuffer(self) -> None:
        ...
        node = Node(
            uid="testuid", httpClient=HttpClientMock(callback=callback), maxBufferSize=1
        )
