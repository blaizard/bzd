...
program.version("1.0.0", "-v, --version")
	...
	.option("-p, --port <number>",
			"Port to be used to serve the application, can also be set with the environemnt variable BZD_PORT.", 8080,
			parseInt)
	.option("-s, --static <path>", "Directory to static serve.", ".")
	.option("-d, --data <path>",
			"Where to store the data, can also be set with the environemnt variable BZD_PATH_DATA.", "/bzd/data")
...
(async () => {
  ...
  let authentication = new Authentication({
	...
	verifyRefreshCallback : async (/*uid, session, timeoutS*/) => { return true; },
  });
  ...
  let services = new Services({
	makeContext : (volume) => {
	  return {
		async getVolume() { return await cache.get("volume", volume); },
	  };
	},
  });
  ...
  // Start all services
  let it = CollectionPaging.makeIterator(
	  async (maxOrPaging) => { return await keyValueStore.list("volume", maxOrPaging); }, 50);
  ...
  web.addRoute("get", "/file(/*)?",
			   async (request, response) => { response.redirect(api.getEndpoint("/file?path=" + request.params[1])); });

  // Adding API handlers.
  function getInternalPath(pathList) {
	Exception.assert(Array.isArray(pathList), "Path must be an array: '{:j}'", pathList);
	Exception.assert(pathList.every((path) => Boolean(path)), "Path elements cannot be empty: '{:j}'", pathList);
	return {volume : pathList[0], pathList : pathList.slice(1)};
  }

  function getInternalPathFromString(path) { return getInternalPath(path.split("/").map(decodeURIComponent)); }
  ...
  api.handle("get", "/config", async (inputs) => { return await keyValueStore.get("volume", inputs.volume, {}); });
  ...
  api.handle("get", "/services", async (/*inputs*/) => { return services.getActive(); });
