...
def getHostPort(hostport: str) -> typing.Tuple[str, int]:
    ...
    assert len(split) == 2, (
        f"The string does not have a correct format, should be 'hostname:port', not '{hostport}'."
    )
...
def commandWol(args: argparse.Namespace) -> None:
    ...
    assert checkMAC(args.mac), (
        f"Mac address '{args.mac}' should have the following format: XX:XX:XX:XX:XX:XX"
    )
...
if __name__ == "__main__":
    ...
    wolParser = subparsers.add_parser(
        "wol", help="Wake-up a machine from its MAC address."
    )
    ...
    suspendParser = subparsers.add_parser(
        "suspend", help="Suspend a machine running the server."
    )
    suspendParser.add_argument(
        "ip", help="The ip:port address for the machine to suspend."
    )

    shutdownParser = subparsers.add_parser(
        "shutdown", help="Shutdown a machine running the server."
    )
    shutdownParser.add_argument(
        "ip", help="The ip:port address for the machine to shutdown."
    )
