...
class Monitor:
    ...
    def batteries(self) -> typing.Any:
        try:
            ...
            return None
        return {"main": [maybeBattery[0] / 100.0]}
        ...
        for partition in psutil.disk_partitions():  # type: ignore
            ...
            if partition.device not in disks:
                ...
                disks[partition.device] = {
                    "path": [],
                    "data": {"used": usage.used, "total": usage.total},
                }
            disks[partition.device]["path"].append(partition.mountpoint)  # type: ignore

        dictionary = {
            ",".join(sorted(data["path"])): data["data"] for data in disks.values()
        }
    ...
    def network(self) -> typing.Any:
        ...
        timestampDiffS = timestampS - self.networkPrevious.get(
        ...
        validInterfaces = {
            interface
            for interface, stats in statsAll.items()
            if stats.isup and "loopback" not in stats.flags.lower()
        }
        ...
        for name, snetio in netStat.items():
            if name in validInterfaces:
                ...
                diffIn = max(
                    snetio.bytes_recv
                    - self.networkPrevious[name].get("in", snetio.bytes_recv),
                    0,
                )
                diffOut = max(
                    snetio.bytes_sent
                    - self.networkPrevious[name].get("out", snetio.bytes_sent),
                    0,
                )
    ...
    def io(self) -> typing.Any:
        ...
        for name, sdiskio in ioStat.items():
            ...
            diffIn = max(
                sdiskio.write_bytes
                - self.ioPrevious[name].get("in", sdiskio.write_bytes),
                0,
            )
            diffOut = max(
                sdiskio.read_bytes
                - self.ioPrevious[name].get("out", sdiskio.read_bytes),
                0,
            )
