...
class Monitor:
	...
	def network(self) -> typing.Any:
		...
		validInterfaces = {
			interface for interface, stats in statsAll.items() if stats.isup and "loopback" not in stats.flags.lower()
		}
		...
		for name, snetio in netStat.items():
			if name in validInterfaces:
				...
				diffIn = max(
					snetio.bytes_recv - self.networkPrevious[name].get("in", snetio.bytes_recv),
					0,
				)
				diffOut = max(
					snetio.bytes_sent - self.networkPrevious[name].get("out", snetio.bytes_sent),
					0,
				)
	...
	def io(self) -> typing.Any:
		...
		for name, sdiskio in ioStat.items():
			...
			diffIn = max(
				sdiskio.write_bytes - self.ioPrevious[name].get("in", sdiskio.write_bytes),
				0,
			)
			diffOut = max(
