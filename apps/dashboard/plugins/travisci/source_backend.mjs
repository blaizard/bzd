...
export default {
	  ...
	  fetch:
		  async (endpoint, repositorySlug, token) => {
			...
			return (result.builds || []).map((item) => {
			  ...
			  return {
				// Duration attribute is not correct as it includes accumulated duration from the jobs eventhough they
				// run in parallel. So we use the actual run time instead.
				duration : Date.parse(item.finished_at) - Date.parse(item.started_at),
				timestamp : Date.parse(item.started_at) || Date.parse(item.finished_at) ||
  ...
  fetch:
	  async (data, cache) => {
		const builds = await cache.get("travisci.builds", data["travisci.endpoint"], data["travisci.repository"],
									   data["travisci.token"]);
		return {
		  builds : builds,
		};
