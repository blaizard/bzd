...
class Snmp {
  ...
  static normalizeOid(oid) { return String(oid).split(".").filter((item) => Boolean(item)).join("."); }

  static isOid(value) { return /^\.?([0-9]+\.){2,}[0-9]+$/.test(value); }
...
export default {
  cache: {
	"snmp.oid": {
	  fetch:
		  async (host, version, community, oids, ttl, previous, options) => {
		  ...
		  async (snmpArray) => {
			...
			const oidTtlMap = snmpArray.reduce((obj, item) => {
			  ...
			  (item.ops || [])
				  ...
				  .forEach((item) => { updateObj(obj, item.value, ttl); });
			  return obj;
			}, {});
  ...
  fetch:
	  async (data, cache) => {
		...
		for (const ttl in oidsByTtl) {
		  ...
		  const promise =
			  cache.get("snmp.oid", data["snmp.host"], data["snmp.version"], data["snmp.community"], oids, ttl);
		  promises.push(promise);
		}

		const values =
			(await Promise.all(promises)).reduce((obj, result) => { return Object.assign(obj, result); }, {});
