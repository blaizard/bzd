...
export default {
  cache: {
	"github.builds": {
	  fetch:
		  async function(username, repository, workflowId, token) {
			...
			const url = baseUrl + (workflowId ? "/actions/workflows/" + encodeURIComponent(workflowId) + "/runs"
											  : "/actions/runs");
	...
	"github.jobs": {
	  fetch:
		  async (url, username, token, prevValue, cacheOptions) => {
  ...
  fetch:
	  async (data, cache) => {
		const builds = await cache.get("github.builds", data["github.username"], data["github.repository"],
									   data["github.workflowid"], data["github.token"]);
		return {
		  builds : builds,
		};
	  },
