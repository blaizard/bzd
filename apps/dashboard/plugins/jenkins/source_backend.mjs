...
export default {
	  ...
	  fetch:
		  async (url, build, branch, user, token) => {
			...
			const result = await HttpClient.get(
				baseUrl + "/api/json?&tree=builds[duration,result,id,timestamp,estimatedDuration]{0,100}", options);
  ...
  fetch:
	  async (data, cache) => {
		const builds = await cache.get("jenkins.builds", data["jenkins.url"], data["jenkins.build"],
									   data["jenkins.branch"] || "master", data["jenkins.user"], data["jenkins.token"]);
		return {
		  builds : builds,
		};
	  },
