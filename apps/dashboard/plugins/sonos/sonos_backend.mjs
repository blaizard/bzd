...
export default {
	...
	"sonos.device": {
	  fetch:
		  async (previous) => {
			...
			const playlist =
				favorites.items.filter((item) => { return "uri" in item; }).map((item) => { return item.uri; });
			...
			instance.on("PlayState", (state) => { track.state = _getState(state); });
  ...
  events : {
	...
	async next(data, cache) { await _next(data, cache, 1);},
	async previous(data, cache) { await _next(data, cache, -1);},
  },
};
