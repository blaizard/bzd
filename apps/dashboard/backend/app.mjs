...
(async () => {
  ...
  api.handle("get", "/tile", async (inputs) => { return await keyValueStore.get("tiles", inputs.uid); });
  ...
  api.handle("delete", "/tile", async (inputs) => { await keyValueStore.delete("tiles", inputs.uid); });
  api.handle("get", "/data", async (inputs) => { return await cache.get(inputs.type, inputs.uid); });
  api.handle("post", "/event", async (inputs) => {
	Exception.assert(inputs.type in events, "No event associated with '{}'.", inputs.type);
	Exception.assert(inputs.event in events[inputs.type], "'{}' is not a valid event for '{}'.", inputs.event,
					 inputs.type);
