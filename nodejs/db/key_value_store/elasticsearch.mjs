...
export default class KeyValueStoreElasticsearch extends KeyValueStore {
  constructor(host, options) {
	...
	Log.info("Using elasticsearch key value store DB at '{}' (prefix: {}, authentication: {}).", host,
			 this.options.prefix, "authentication" in fetchOptions ? "on" : "off");
  ...
  _bucketToURI(bucket) { return this.options.prefix + encodeURIComponent(bucket); }
  ...
  async _setImpl(bucket, key, value, query = {}) {
	...
	Exception.assert([ "created", "updated" ].includes(result.result), "Unexpected result, received: {}",
					 result.result);
  ...
  async _updateImpl(bucket, key, modifier, defaultValue, maxConflicts) {
	...
	Exception.unreachable("Too many conflicts have occured with update request ({}, {}), something is wrong, aborting.",
						  bucket, key);
  }
  ...
  async _search(bucket, maxOrPaging, query) {
	...
	try {
	  ...
	  return CollectionPaging.makeFromTotal(result.hits.hits.reduce((obj, item) => {
		obj[item._id] = item._source;
		return obj;
	  }, {}), paging, result.hits.total.value);
	} catch (e) {
	  if (e instanceof HttpClientException) {
		if (e.code == 404 /*Not Found*/) {
