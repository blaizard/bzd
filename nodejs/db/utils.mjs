export class CollectionPaging {
  ...
  isLast() { return !this._nextPaging; }

  getNextPaging() { return this._nextPaging; }
  ...
  static makeFromTotal(data, paging, total) {
	return new CollectionPaging(
		data, total > (paging.page + 1) * paging.max ? {page : paging.page + 1, max : paging.max} : null);
  }
...
export class AsyncInitialize {
   ...
   * Initialize (optional)
   */
  async _initialize() { await this._initializeImpl(); }
}
