...
export default class StorageDockerV2 extends Storage {
  ...
  async authenticationScope(service, scope, fetchOptions) {
	return this.fetch.get("/v2/token", Object.assign({
	  query : {
		service : service,
		scope : scope,
	  },
  ...
  async _listCatalog(maxOrPaging) {
	return await this._listPagination("/v2/_catalog", maxOrPaging, "registry:catalog:*", (result) => {
	  return result.repositories.map((item) => Permissions.makeEntry({
		name : item,
		type : "directory",
	  },
																	 {
  ...
  async _listTags(imageName, maxOrPaging) {
	return await this._listPagination("/v2/" + imageName + "/tags/list", maxOrPaging,
									  "repository:" + imageName + ":pull", (result) => {
										return result.tags.map((item) => Permissions.makeEntry({
										  name : item,
										  type : "directory",
										},
																							   {
  ...
  async _listLayers(imageName, tag, maxOrPaging) {
	...
	data.unshift(Permissions.makeEntry({
	  name : "manifest.json",
