...
export default class StorageDisk extends Storage {
  ...
  _getFullPath(pathList) { return Path.join(this.path, ...pathList); }

  async _isImpl(pathList) { return await FileSystem.exists(this._getFullPath(pathList)); }

  async _readImpl(pathList) { return Fs.createReadStream(this._getFullPath(pathList)); }
  ...
  async _deleteImpl(pathList) { await FileSystem.unlink(this._getFullPath(pathList)); }
