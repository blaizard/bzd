...
def inputDeserialize(
    data: bytes, callback: typing.Callable[[ActionType, typing.Any], None]
) -> bytes:
    ...
    while proc.poll() is None:
        ...
        if sys.stdin.fileno() in rlist:
            try:
                ...
                if inputData:
                    inputBuffer = inputDeserialize(
                        inputBuffer + inputData, inputProcess
                    )
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-t", "--term", default="xterm-color", type=str, help="The terminal name."
    )
