...
class RouterManager {
  constructor(options) {
	...
	this.path = "/";
	this.regexMatchPath =
		new RegExp(this.options.hash ? "^[^#]+(?:#([^?]+))?(?:\\?.*)?$" : "^.*://[^/]+/([^?#]*)(?:[?#].*)?$");
  }

  _getUid(vueElt) { return String(vueElt._uid); }
  ...
  _makeRouter(vueElt, options) {
	let router = new Router({
	  fallback : (path) => { Log.error("Route '{}' not found", path); },
	});
	...
	options.routes.forEach((route) => {
	  const handler = async (route, args, path, routerOptions) => {
		...
		if (route.authentication) {
		  Exception.assert(this.options.authentication,
	  ...
	  router.add(route.path, (args, routerOptions) => { handler(route, args, "/", routerOptions); });
  ...
  async registerRouter(vueElt, options) {
	...
	Log.debug("Registered router '{}' with parent '{}' (nb routers: {})", uid, this.routers.get(uid).parent,
			  this.routers.size);
  ...
  unregisterRouter(uid) {
	...
	children.forEach((childUid) => { this.unregisterRouter(childUid); });
	if (this.routers.get(uid).parent) {
	  let parent = this.routers.get(this.routers.get(uid).parent);
	  Exception.assert(parent.children.delete(uid), "Child '{}' was not registered in parent '{}'", uid,
  ...
  async dispatch(path = null, options = {}) {
	...
	const queryStr =
		Object.keys(options.query).length
			? "?" +
				  Object.keys(options.query).map((key) => key + "=" + encodeURIComponent(options.query[key])).join("&")
			: "";
  ...
  async _propagate(uid, options = {}) {
	...
	router.path =
		data.path.substr(0, data.path.length - router.pathPropagate.length + (router.pathPropagate == "/" ? 1 : 0));

	Log.debug("Router '{}' processed path '{}' (with vars {:j}) and propagate path '{}'", uid, router.path, data.vars,
	...
	let promiseList =
		Array.from(router.children).map(async (childUid) => { await this._propagate(childUid, options); });
	await Promise.all(promiseList);
  }
}
...
export default class {
  static install(Vue, options) {
	...
	Vue.prototype.$routerDispatch = async (path, options = {}) => { await routers.dispatch(path, options); };

	Vue.prototype.$routerGet = () => { return routers.getRoute(); };
