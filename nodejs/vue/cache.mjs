...
function idsToId(...ids) { return ids.map((id) => String(id)).join("/") || "default"; }
	...
	Vue.prototype.$cache = new Vue({
	  ...
	  methods : {
		getReactive(collection, ...ids) {
		  ...
		  cache.get(collection, ...ids).then((value) => { this.content[collection][id] = value; });

		  return this.content[collection][id];
		},
		async get(collection, ...ids) { return await cache.get(collection, ...ids); },
