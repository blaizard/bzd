...
export default {
  install(Vue) {
	Vue.prototype.$notification = new Vue({
	  ...
	  computed : {
		defaultActions() { return [ {id : "close", callback : (entry) => this.close(entry)} ]; },
	  },
	  methods : {
		...
		_notifySingle(key, type, message, options) {
		  ...
		  if (messageStr === null) {
			...
			setTimeout(() => { this.close(entry); }, entry.timeOnScreen * 1000);
		  }
		},
		/**
		...
		info(message, options) { this._notify("info", message, options); },
		...
		success(message, options) { this._notify("success", message, options); },
		close(entry) {
