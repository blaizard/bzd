...
function registerObserver(el, callback) {
  ...
  if (typeof ResizeObserver !== "undefined") {
	observers[uid] = {
	  obj : new ResizeObserver((/*entries*/) => { onResizeProcess(el); }),
	  callback : callback,
	  hash : -1,
	  instance : null,
	};
	...
	observers[uid] = {
	  obj : setInterval(() => { onResizeProcess(el); }, 1000),
	  callback : callback,
	  hash : -1,
	  instance : null,
	};
...
function getElementId(el) { return el.getAttribute("data-irresize") || null; }
...
export default {
  inserted: function(el, binding) { onResizeProcess(el, binding.value);},
  componentUpdated: function(el, binding) { onResizeProcess(el, binding.value);},
  unbind:
	  function(el) {
