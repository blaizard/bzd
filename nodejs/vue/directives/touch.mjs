...
async function dragStart(e) {
  ...
  if (curConfig.drag) {
	...
	if (curConfig.selectorReceiver) {
	  ...
	  receiverEltList.forEach((elt) => { elt.classList.add(DRAG_RECEIVER_ACTIVE); });
...
function isPointInRectangle(point, rectangle, tolerance) {
  return (rectangle.left - tolerance <= point.x && rectangle.top - tolerance <= point.y &&
...
Touch.prototype.handleEvent = function(e) { activateTouch(e, this.el, this.config); };
