...
function autoResize(el) {
  ...
  observers[uid].instance = setTimeout(() => {
	...
	do {
	  ...
	  // console.log(uid + "; max: [" + rectParent.width + "px, " + rectParent.height + "px], size: [" + rect.width +
	  // "px, " + rect.height + "px], font: [" + minFontSize + "px, " + maxFontSize + "px]"); The difference is not an
	  // integer, therefore we must check the upper limit (hence the >=2 and not > 1)
...
function registerObserver(el) {
  ...
  if (typeof ResizeObserver !== "undefined") {
	observers[uid] = {
	  obj : new ResizeObserver((/*entries*/) => { autoResize(el); }),
	  hash : -1,
	  instance : null,
	};
	...
	observers[uid] = {
	  obj : setInterval(() => { autoResize(el); }, 1000),
	  hash : -1,
	  instance : null,
	};
  }
...
function getElementId(el) { return el.getAttribute("data-auto-size") || null; }
...
export default {
  inserted: function(el) { autoResize(el);},
  componentUpdated: function(el) { autoResize(el);},
  unbind:
	  function(el) {
