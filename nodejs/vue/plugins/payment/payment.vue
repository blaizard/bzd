<template>
    <div class="bzd-payments">
		<component v-for="payment, index in payments" :key="index" :is="componentList[index]" :amount="amount" :metadata="metadata"></component>
    </div>
</template>
...
export default {
	...
	methods: {
		getComponent(payment) {
			return () => payment.getComponent(Vue,
				() => { this.$notification.success({"bzd-payment": "Payment received"}); },
				(message = null) => { this.$notification.error({"bzd-payment": message}); });
		}
	}
};
</script>
