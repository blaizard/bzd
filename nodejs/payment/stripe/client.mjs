...
export default class StripePayment {
  ...
  installAPI(api) {
	...
	if (!this.options.getMetadata) {
	  this.options.getMetadata = async () => { return await api.request("get", "/payment/stripe/metadata"); };
	}
  ...
  async getComponent(Vue, handleSuccess = () => {}, handleError = () => {}) {
	...
	return Vue.extend({
	  mixins : [ VueComponent ],
	  methods : {
		async makePaymentIntent(amount, metadata) { return await makePaymentIntent(amount, metadata); },
		async getMetadata() { return await getMetadata(); },
