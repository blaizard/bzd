...
class Cache {
  ...
  register(collection, trigger, options) {
	Exception.assert(typeof collection === "string", "register(...) must take a string as first argument: {:j}",
					 collection);
	Exception.assert(typeof trigger === "function", "register(...) must take a function as second argument: {:j}",
					 trigger);
	Exception.assert(collection.match(/^[a-z0-9.-_]+$/i),
   ...
   */
  isCollection(collection) { return collection in this.data; }

  /**
   * Current timestamp
   */
  static getTimestampMs() { return Date.now(); }
   ...
   */
  async get(collection, ...ids) { return get.call(this, /*instant*/ false, collection, ...ids); }
   ...
   * \return The data or a promise that will return the data.
   */
  async getInstant(collection, ...ids) { return get.call(this, /*instant*/ true, collection, ...ids); }

  /**
   * \brief Check if the ressource is dirty
   */
  isDirty(collection, ...ids) { return isDirty.call(this, collection, ...ids); }
...
function idsToId(collection, ...ids) {
  ...
  Exception.assert(typeof this.data[collection] === "object", "Cache::get({}) does not exist or is not of type object.",
				   collection);
	...
	if (isFetching.call(this, collection, ...ids)) {
	  ...
	  if (instant) {
		...
		// isData
		  ...
		  Log.debug("GET {}::{} ({}, instant)", collection, id,
 ...
 */
function touchResource(collection, id) { this.resourceMap[collection + "/" + id] = Cache.getTimestampMs(); }
 ...
 */
async function waitForNextUpdate() { return this.event.waitUntil("updated"); }
...
async function triggerUpdate(collection, id, ...ids) {
  ...
  try {
	Exception.assert(typeof dataCollection._trigger === "function",
					 "No trigger function associated with collection '{}'", collection);
	...
	dataId._data = await dataCollection._trigger.call(this, ...ids,
	...
	Exception.assert(typeof dataId._data !== "undefined",
					 "Trigger function returned undefined data type for collection '{}'", collection);
...
async function garbageCollector() {
  ...
  try {
	while (this.data._size > this.config.maxSize ||
	  ...
	  Log.info("GARBAGE COLLECTION size={}, nbEntries={}: removing entry '{}'", this.data._size, this.data._nbEntries,
			   entry[0]);
	...
	this.interval = setTimeout(() => { garbageCollector.call(this); }, this.config.garbageCollectorPeriodMs);
  }
}
