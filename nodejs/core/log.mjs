...
class Performance {
  constructor() { this.timeStart = Date.now(); }
...
class Logger {
  ...
  setLevel(level) { this.config.level = Logger.levels[level].level; }
...
const custom = (config, ...msgs) => { logger.print(config.level, config.topics, ...msgs); };
...
const LoggerFactory = (...topics) => {
  return {
	...
	getLevel() { return logger.getLevel(); },
	mute() { logger.setLevel("error"); },
