...
class TaskId {
  ...
  isEqual(taskId) { return taskId.namespace === this.namespace && taskId.name === this.name; }

  toString() { return this.namespace + "::" + this.name; }
}
...
export default class Task {
  constructor(namespace, name, action, config) {
	...
	// Sanity check
	Exception.assert(typeof this.config.priority === "number",
					 () => "config.priority must be a number, instead: " + JSON.stringify(this.config.priority));
	Exception.assert(this.config.priority >= Task.PRIORITY_LOWEST && this.config.priority <= Task.PRIORITY_HIGHEST,
					 () => "config.priority is not in the range, instead: " + JSON.stringify(this.config.priority));
	Exception.assert(typeof this.action === "function",
					 () => "Task " + this.toString() +
						   " action must be a function, instead: " + JSON.stringify(this.action));
  }
   ...
   */
  isEqual(task) { return this.id.isEqual(task.id); }
  ...
  get isValid() { return this.config.isValid(this.iteration); }

  get delayMs() { return this.config.delayMs || 0; }

  get intervalMs() { return this.config.intervalMs || 0; }
