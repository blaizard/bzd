...
export default class Manager {
   ...
   */
  isRegistered(taskId) { return taskId.namespace in this.tasks && taskId.name in this.tasks[taskId.namespace]; }
  ...
  scheduleTaskIfValid(task, delayMs, triggerTaskScheduler) {
	...
	Exception.assert(task.status === Task.STATUS_IDLE,
  ...
  getNextTaskTimestamp() {
	const minTimestamp = this.taskQueues.reduce(
		(minTimestamp, queue) => { return queue.length ? Math.min(queue[0].timestamp, minTimestamp) : minTimestamp; },
		Number.MAX_VALUE);
  ...
  triggerTaskScheduler() {
	...
	if (nextTimestamp !== null) {
	  ...
	  if (!this.schedulerInstance) {
		this.schedulerInstance =
			setTimeout(() => { this.taskScheduler(); }, nextTimestamp > timestamp ? nextTimestamp - timestamp : 1);
