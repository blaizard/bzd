...
class Constraint {
  constructor(schema, key, name, args) {
	...
	Exception.assert(this.isValidNbArgs(this.args.length), "Wrong number of arguments passed to '{}': {}", this.name,
					 this.args.length);

	this.create();
  }

  get entry() { return this.schema[this.key]; }
  ...
  assert(result, condition, str, ...args) { return Constraint.assert(result, this.key, condition, str, ...args); }
}
...
export default class Validation {
  ...
  validate(values, options) {
	...
	for (const key in this.schema) {
	  if (Constraint.assert(result, key,
	...
	case "throw": {
	  ...
	  if (keys.length == 1) {
		...
		const message = keys.map((key) => { return String(key) + ": (" + result[key].join(", ") + ")"; });
  ...
  isMandatory(name) { return name in this.schema ? this.schema[name].mandatory : false; }

  _processConstraint(schema, key, name, args) {
	const singleValue = this.singleValue;
	const availableConstraints = {
	  mandatory : class Mandatory extends
	  Constraint{isValidNbArgs(n) { return n == 0; } create() { this.entry.mandatory = true; }},
	  values : class Values extends Constraint{
		isValidNbArgs(n) { return n > 0; } install() {
		  this.castArgsToType();
		  return (result, value) => {
		...
		isValidNbArgs(n) { return n == 1; } install() {
		  this.castArgsToType();
		  switch (this.entry.type) {
		  case "string":
			return (result, value) => {
		  ...
		  case "float":
			return (result,
					value) => { this.assert(result, value >= this.args[0], "greater or equal to {}", this.args[0]); };
		  default:
		...
		isValidNbArgs(n) { return n == 0; } install() {
		  ...
		  return (result, value) => {
			this.assert(result, value.match(/^([a-zA-Z0-9_\-.]+)@([a-zA-Z0-9_\-.]+)\.([a-zA-Z]{2,5})$/), "email");
																						...
																						type : class Type extends
	  Constraint{
		isValidNbArgs(n) { return n == 1; } create() {
		  Exception.assert([ "string", "integer", "float", "boolean" ].includes(this.args[0]),
						   "'{}' is not a supported type", this.args[0]);
