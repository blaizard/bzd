...
export default class TokenAuthenticationServer extends AuthenticationServer {
  ...
  _installAPIImpl(api) {
	...
	api.handle("post", "/auth/logout", async function() { this.deleteCookie("refresh_token"); });
  ...
  async generateAccessToken(data) { return await this._generateToken(data, this.options.tokenAccessExpiresIn); }

  async generateRefreshToken(uid, roles, persistent, session) {
	return await this._generateToken({uid : uid, roles : roles, persistent : persistent, session : session},
									 persistent ? this.options.tokenRefreshLongTermExpiresIn
