...
export default class TokenAuthenticationClient extends AuthenticationClient {
  ...
  _isAuthenticatedImpl() { return this.token !== null; }
  ...
  _installAPIImpl(api) {
	...
	if (!this.options.refreshTokenCallback) {
	  this.options.refreshTokenCallback = async () => { return await api.request("post", "/auth/refresh"); };
	}
  ...
  setToken(token, timeoutS = 0) {
	...
	if (token && timeoutS && this.options.refreshTokenCallback) {
	  ...
	  this.interval = setInterval(() => { this.refreshAuthentication(); }, Math.max(timeoutS - 60, 30) * 1000);
	}
  }
