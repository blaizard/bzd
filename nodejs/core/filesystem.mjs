...
export default class FileSystem {
  ...
  static async exists(path) {
	return new Promise((resolve) => { Fs.access(path, Fs.F_OK, (e) => { resolve(e ? false : true); }); });
  }

  /// Get the stat object associated with a file
  static async stat(path) {
	return new Promise(
		(resolve, reject) => { Fs.stat(path, (e, stats) => { return e ? reject(e) : resolve(stats); }); });
  }
  ...
  static async move(pathFrom, pathTo) { await Fs.promises.rename(pathFrom, pathTo); }
  ...
  static async copy(pathFrom, pathTo) { await Fs.promises.copyFile(pathFrom, pathTo, Fs.constants.COPYFILE_EXCL); }

  /// Delete a file
  static async unlink(path) { await Fs.promises.unlink(path); }

  /// Changes the permissions of a file.
  static async chmod(path, mode) { await Fs.promises.chmod(path, mode); }

  /// Append data to a file
  static async appendFile(path, data) { await Fs.promises.appendFile(path, data); }

  /// Truncate a file to a specific size
  static async truncate(path, fileSize) { await Fs.promises.truncate(path, fileSize); }
  ...
  static async open(path, options) { return await Fs.promises.open(path, options); }

  /// Close a file asynchronously, previously open with "open"
  static async close(fileHandle) { await fileHandle.close(); }

  /// Determines the actual location of a path and resolve symlinks.
  static async realpath(path, options) { return await Fs.promises.realpath(path, options); }

  /// Creates a symbolic link.
  static async symlink(target, path) { return await Fs.promises.symlink(target, path); }

  /// Creates a hard link.
  static async hardlink(target, path) { return await Fs.promises.link(target, path); }
