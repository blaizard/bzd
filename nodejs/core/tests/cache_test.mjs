...
describe("Cache", () => {
  describe("Simple", () => {
	it("Base", async () => {
	  ...
	  await Exception.assertThrows(async () => { await cache.get("test", false); });
  ...
  it("Stress", async () => {
	...
	let expected = 0;
	cache.register("test", (arg) => {
	  return new Promise((resolve) => { setTimeout(() => { resolve(parseInt(arg)); }, Math.random() * 10); });
	}, {
	  timeout : 10,
	});

	const promiseDelay = (t) => { return new Promise((resolve) => { setTimeout(resolve, t); }); };
