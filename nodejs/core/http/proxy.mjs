...
export default class HttpProxy {
  constructor(target, port, options) {
	...
	Exception.assert([ "http:", "https:" ].includes(this.target.protocol), "Protocol not supported '{}'",
  ...
  async start() {
	this.server = Http.createServer(async (request, response) => {
	  ...
	  request.on("aborted", () => { Log.info("ABORT request {}/{}", this.target.hostname, request.url); });

	  response.on("aborted", () => { Log.info("ABORT response {}/{}", this.target.hostname, request.url); });
  ...
  async stop() { await this._closeServer(); }
}
