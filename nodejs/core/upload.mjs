...
class UploadItem {
  ...
  get totalBytes() { return this.file.size; }
...
export default class Upload {
  ...
  async trigger(nothrow, options) {
	...
	return new Promise((resolve, reject) => {
	  ...
	  if (this.config.filter) {
		...
		input.setAttribute("accept", filterList
										 .map((filter) => {
										   switch (filter) {
										   case "image":
											 return "image/*";
	  ...
	  setTimeout(() => { document.addEventListener("mousemove", resolveCancel, {once : true}); }, 1000);
	});
  }
  ...
  async handleFiles(files, nothrow) {
	const maxNbFiles =
		Math.min(files.length, typeof this.config.max === "function" ? this.config.max() : this.config.max);
	let promiseList = [];
		...
		xhr.onreadystatechange = () => {
		  if (xhr.readyState === 4) {
			if (xhr.status === 200) {
			  ...
			  item.error = xhr.statusText + " (" + xhr.status + ")" +
						   ": " + xhr.response;
			  handleError();
			}
		  }
