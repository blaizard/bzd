...
export default class TimeSeries {
  ...
  forEach(callback, timestampStart, timestampEnd, inclusive = false) {
	...
	let index =
		this.find(timestampStart, inclusive ? TimeSeries.FIND_IMMEDIATELY_BEFORE : TimeSeries.FIND_IMMEDIATELY_AFTER);
	...
	if (this.options.unique) {
	  ...
	  for (let index = 1; index < this.data.length; ++index) {
		...
		if (this.data[index - 1][0] == this.data[index][0]) {
		  this.data[index][1] =
			  this.options.uniqueMerge(this.data[index][1], this.data[index - 1][1], this.data[index][0]);
		  indexToDeleteList.push(index - 1);
		}
	  }
	  // Delete entries from the list
	  indexToDeleteList.reverse().forEach((index) => { this.data.splice(index, 1); });
	}
  }
