...
export default class APIClient extends Base {
  ...
  async request(method, endpoint, data) {
	...
	if ("validation" in requestOptions) {
	  Exception.assert([ "json", "query" ].includes(requestOptions.type), "{} {}: validation is not available for {}.",
					   method, endpoint, requestOptions.type);
	...
	do {
	  ...
	  if (this.schema[endpoint][method].authentication) {
		Exception.assert(this.isAuthentication(),
	  ...
	  try {
		...
		if ("validation" in responseOptions) {
		  Exception.assert([ "json" ].includes(responseOptions.type), "{} {}: validation is not available for {}.",
						   method, endpoint, responseOptions.type);
