...
export default class API {
  constructor(schema, options) {
	...
	this.options.plugins.forEach((plugin) => { plugin.installAPI(this); });
  }
   ...
   */
  addSchema(schema) { Object.assign(this.schema, schema); }

  /**
   * If authentication is supported
   */
  isAuthentication() { return this.options.authentication !== null; }

  getEndpoint(endpoint) { return "/api/v" + this.options.version + endpoint; }
  ...
  _sanityCheck(method, endpoint) {
	...
	Exception.assert(method in this.schema[endpoint], "The method '{}' is not valid for endpoint '{}'", method,
					 endpoint);
  }
}
