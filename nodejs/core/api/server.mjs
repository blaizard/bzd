...
class Context {
  ...
  getEndpoint(endpoint) { return new Url.URL(this.api.getEndpoint(endpoint), this.getHost()).href; }

  setHeader(key, value) { this.response.setHeader(key, value); }
  ...
  getCookie(name, defaultValue) { return name in this.request.cookies ? this.request.cookies[name] : defaultValue; }
  ...
  addDebug(name, data) { this.debug[name] = data; }
...
export default class APIServer extends Base {
  ...
  handle(method, endpoint, callback /*, options = {}*/) {
	...
	Log.debug("Installing handler for {} {}{}", method, endpoint,
	...
	const callbackWrapper = async (request, response) => {
	  ...
	  try {
		...
		if ("validation" in requestOptions) {
		  Exception.assert([ "json", "query" ].includes(requestOptions.type),
						   "{} {}: validation is not available for {}.", method, endpoint, requestOptions.type);
		...
		if ("validation" in responseOptions) {
		  Exception.assert(responseOptions.type == "json", "{} {}: validation is only available for json type.", method,
						   endpoint);
		...
		if (!context.manualResponse) {
		  ...
		  case "json":
			Exception.assert(typeof result == "object", "{} {}: callback result must be a json object.", method,
							 endpoint);
		  ...
		  case "file":
			if (typeof result == "string") {
			  ...
			  await new Promise(
				  (resolve,
				   reject) => { result.on("error", reject).on("end", resolve).on("finish", resolve).pipe(response); });
			  response.end();
			} else {
