...
import TaskManager from "../task/manager_singleton.mjs";

const Log = LogFactory("persistence", "disk");
const Exception = ExceptionFactory("persistence", "disk");
...
export default class PersistenceDisk {
  constructor(path, options) {
	this.options = Object.assign({
	  ...
	  operations : {
		 ...
		 */
		set : (data, key, value) => { data[key] = value; },
		/**
   ...
   */
  get size() { return this.estimatedSize; }
  ...
  async _initialize(ignoreErrors) {
	...
	try {
	  ...
	  if (this.options.savepointTask !== null) {
		...
		this.savepointTaskId =
			TaskManager.register(taskConfig.namespace, taskConfig.name, () => this.taskSavepoint(), taskConfig);
	  }
	} catch (e) {
  ...
  async initializeDelta(/*ignoreErrors*/) {
	...
	delta.list = fileList.map((name) => { return parseInt(name.substring(0, name.indexOf("."))); })
					 .sort((a, b) => { return a - b; });
   ...
   */
  async waitReady() { return await this.event.waitUntil("ready"); }

  /**
   * Check if persistence is ready
   */
  isReady() { return this.event.is("ready"); }
  ...
  async write(type, ...args) {
	Exception.assert(type in this.options.operations, () => "The operation \"" + type +
   ...
   */
  getPathFromId(id) { return this.delta.dir + id + ".log"; }
  ...
  async consistencyCheck() {
	...
	try {
	  ...
	  Exception.assert(delta.list.length == this.delta.list.length,
					   () => "Deltas on disk " + JSON.stringify(delta.list) + " differs from deltas in memory" +
							 JSON.stringify(this.delta.list));
	  Exception.assert(delta.list.every((id, index) => this.delta.list[index] == id),
					   () => "Deltas on disk " + JSON.stringify(delta.list) + " differs from deltas in memory" +
							 JSON.stringify(this.delta.list));
	  ...
	  Exception.assert(this.delta.dirty || delta.dirty == this.delta.dirty,
