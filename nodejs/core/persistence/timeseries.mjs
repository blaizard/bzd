...
const Log = LogFactory("persistence", "timeseries");
const Exception = ExceptionFactory("persistence", "timeseries");
...
export default class PersistenceTimeSeries {
  ...
  constructor(path, options) {
	...
	this.timeSeriesIndex = new TimeSeries({
	  ...
	  uniqueMerge : (a, b, timestamp) => {
		Exception.unreachable("Duplicate timestamps (" + timestamp + ") are in the index for entries: " + a + " and " +
							  b);
	  },
	});
  ...
  async forEach(callback, timestampStart, timestampEnd, inclusive) {
	...
	do {
	  const result = await this.getPersistenceByTimestamp(
		  timestamp,
		  /*forWrite*/ false, isFirst ? TimeSeries.FIND_IMMEDIATELY_BEFORE : TimeSeries.FIND_IMMEDIATELY_AFTER);
  ...
  async getTimestamp(index) {
	...
	return await this.timeSeriesData.wrap((await result.persistence.get()).list,
										  (timeseries) => { return timeseries.getTimestamp(result.index); });
  }
  ...
  async rebuildIndex() {
	...
	// List all data files
	const fileList =
		(await FileSystem.exists(this.options.dataDir)) ? await FileSystem.readdir(this.options.dataDir) : [];
	  ...
	  operations : {
		insert :
			async (data, timestamp,
				   value) => { await this.timeSeriesIndex.wrap(data.list, (t) => { t.insert(timestamp, value); }); },
		update : async (data, timestamp) => {
		  await this.timeSeriesIndex.wrap(data.list, (t) => {
			...
			// Increase the length
			Exception.assert(typeof t.data[index] == "object", () => "Cannot find timestamp " + timestamp +
																	 " in index, returned index: " + index + " " +
			...
			Exception.assert("timestampEnd" in t.data[index][1], () => "No timestampEnd property for index entry " +
																	   index + "  " + JSON.stringify(t.data[index]));
  ...
  get optionsDataReadOnly() {
	return {
	  ...
	  operations : {
		insert :
			async (data, timestamp,
				   value) => { await this.timeSeriesData.wrap(data.list, (t) => { t.insert(timestamp, value); }); },
	  },
	};
  }
  ...
  async getPersistenceByTimestamp(timestamp, forWrite = false, mode = TimeSeries.FIND_IMMEDIATELY_BEFORE) {
	Exception.assert(typeof timestamp == "number",
	...
	Exception.assert(this.persistenceCache[metadata.path] instanceof PersistenceDisk,
   ...
   * \param forWrite If set, it will create a file if needed. If not set, it needs to ensure that the timestamp is
   * valid. \param mode The search mode if no exact match.
   *
  ...
  async getMetadata(timestamp, forWrite, mode = TimeSeries.FIND_IMMEDIATELY_BEFORE) {
	return await this.timeSeriesIndex.wrap((await this.persistenceIndex.get()).list, async (timeSeriesIndex) => {
	  ...
	  if (index != -1) {
		...
		// If the index is found and the index is already full and the requested timestamp should be placed at the end
		// of the file, then create a new file
